...
class TrainFromCorrectionsTests(unittest.TestCase):
    ...
    def test_dry_run_report_exposes_machine_readable_readiness(self) -> None:
        ...
        self.assertEqual(report["summary"]["recommended_label"], "A")
        self.assertEqual(report["summary"]["recommended_batch_labels"], ["A", "a"])
        self.assertEqual(report["summary"]["not_ready_label_count"], 2)
        self.assertEqual(report["summary"]["not_ready_label_list"], ["A", "a"])
        self.assertEqual(report["summary"]["recommended_batch_size"], 2)
        self.assertEqual(report["summary"]["recommended_batch_samples"], 2)
        self.assertEqual(report["summary"]["recommended_batch_target_samples"], 40)
        self.assertEqual(report["summary"]["recommended_batch_needed_samples"], 38)
        self.assertAlmostEqual(report["summary"]["recommended_batch_coverage_percent"], 5.0)
        self.assertEqual(report["summary"]["training_blocked_reason"], "Need 38 more labeled samples across 2 labels before training.")
        self.assertEqual([item["queue"] for item in report["queue_recommendations"]], ["mixedcase", "character", "folded_alnum"])
        self.assertEqual(report["queue_recommendations"][0]["recommended_label"], "A")
        self.assertEqual(report["queue_recommendations"][1]["recommended_label"], "-")
        ...
        self.assertEqual(report["folded_alnum"]["not_ready_label_count"], 1)
        ...
        self.assertEqual(report["mixedcase"]["not_ready_label_count"], 2)
