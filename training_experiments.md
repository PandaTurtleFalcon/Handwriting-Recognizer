...
- Rejected T-H-E Dataset Version IV mixed-case probe:
  ...
  - Code path: added `scripts/prepare_the_dataset.py` to download/convert Version IV into the existing mixed-case `images`/`targets` tensor-cache format. The converter maps T-H-E class ids `1-26` to `a-z` and `40-65` to `A-Z`, accepts binary/int/float pixel rows, and normalizes glyphs through the same 28x28 foreground transform used by other mixed-case extras.
  ...
  - Result: rejected/restored. The epoch ended at exact `80.93%`, digits `98.92%`, upper `71.46%`, lower `83.38%`; lowercase improved, but uppercase collapsed and exact stayed below the deployed acceptance floor. The trainer raised and the wrapper restored artifacts.
  - Verification: `test_prepare_the_dataset.py` passed (`4` tests), `test_prepare_the_dataset.py test_extra_alnum_datasets.py` passed (`39` tests), and post-restore `scripts/summarize_benchmarks.py --include-uploaded-hardcases --json` returned the deployed baseline.
  - Takeaway: T-H-E has useful lowercase signal, but mixing it directly with frozen features is not deployable. Next attempt should use a domain-balancing sampler or case-specific adapter/distillation route that preserves uppercase anchors while borrowing lowercase shape variety.
