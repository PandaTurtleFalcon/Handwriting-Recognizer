...
- Rejected confirmed mixed-case case-resolver probes:
  ...
  - Takeaway: a case/type resolver is still the right problem shape because folded letter identity is `95.75%` accurate and oracle case replacement could add more than four exact points. Embeddings make selection/confirmation more plausible, but confirmed thresholds still do not generalize to the untouched test split. The next attempt should use a stricter validation-style selection split or train a resolver with an explicit balanced upper/lower objective in the loss itself, not just threshold ranking.
