...
- Mixed-case folded-identity hybrid and reported screenshot replay:
  ...
  - Hybrid result: the rejected naive all-glyph folded-identity hybrid fell to about `53.75%` calibrated exact because folded identity failed on EMNIST ByClass digit samples. The accepted gated hybrid keeps mixed-case predictions for digits and uses folded alnum identity only for letters, improving verified mixed-case exact from `87.53%` to `91.28%`, case-or-visual from `97.78%` to `98.28%`, upper exact to `92.73%`, and lower exact to `80.22%`; digit exact stays `95.02%`. App gates remain green after the exact hardcase guards: clean `100.00% (45/45)` and script `96.67% (87/90)`.
  - Remaining blocker: this is a real improvement but still below the `95%` exact goal. Lowercase exact remains the largest mixed-case gap, and should not be hidden by the hybrid artifact.
