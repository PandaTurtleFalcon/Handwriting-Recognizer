...
- Rejected confirmed mixed-case case-resolver probes:
  ...
  - Takeaway: a case/type resolver is still the right problem shape because folded letter identity is `95.75%` accurate and oracle case replacement could add more than four exact points. Embeddings make selection/confirmation more plausible, but the selected gate still does not generalize to the untouched test split. The next attempt should train a resolver with an explicit balanced upper/lower objective or group-floored selection criterion, not just append more passive features.
