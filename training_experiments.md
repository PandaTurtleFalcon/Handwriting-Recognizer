...
- Rejected confirmed mixed-case case-resolver probes:
  ...
  - Takeaway: a case/type resolver is still the right problem shape because folded letter identity is `95.75%` accurate and oracle case replacement could add more than four exact points. The current logits/geometry resolver is not stable enough; the next attempt should add learned image embeddings or train a resolver with an explicit identity+case multitask objective.
