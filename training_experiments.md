...
- Mixed-case top-six reranker speed controls and rejected capped smoke:
  ...
  - Cached feature-sweep harness: `scripts/sweep_mixedcase_feature_reranker.py` now prepares train/test model outputs once per sweep and reuses them across parameter rows. A three-row cached `1Iil` smoke completed with `prepared_once=true` and `0` promotable rows; the closest final candidate moved exact from `87.7782%` to `87.7797%` and digit from `95.0249%` to `95.0278%`, but missed the explicit case-or-visual floor, so no artifact was written.
  - Rejected top-six cached sweep: a twelve-row cached run over `!/1Iil|`, `0Oo`, `5Ss`, `MNmn`, `9gq`, and `Uuv` completed from `96` planned rows with `0` promotable candidates. `scripts/summarize_mixedcase_probes.py` now reports whole-grid family rejection counts; this run failed mostly at final gates (`30` case-or-visual floor misses and `24` final delta misses), with fewer selection (`13`) and confirmation (`5`) misses. The largest final exact move was only `+0.0024` from `Uuv`, and it lowered the protected lower-case split.
  - Rejected lower-case greedy-bias probe: a dry-run greedy per-label bias search over the same top-six family labels, optimizing `lower_test_accuracy` while including the current pair rules, found `0` safe steps. The report now records `includes_pair_rules`, `pair_rule_count`, and the pair-rule artifact hash so future dry runs are easier to compare against the deployed hybrid stack.
  - Takeaway: the faster harness can now run bounded high-dimensional probes, but the top-six path is not blocked by training-loop speed alone. It needs split-aware calibration or a stronger case/visual resolver that preserves case-or-visual ambiguity and lower/upper floors, not broader versions of the same small adapter or bias sweep.

- Character reranker speed controls and rejected stability checks:
  - Code path: `scripts/probe_character_family_reranker.py` now supports balanced `--max-probe-train-samples` caps and `--mini-batch-size`; `scripts/sweep_character_family_reranker.py` forwards and reports those knobs. This mirrors the mixed-case sweep controls so top-family character probes can be tested without full-batch training on every row.
  - Rejected unstable punctuation-source probe: a top-three sweep over `!/1Iil|`, `0Oo`, and `5Ss` found one tiny promotable `!/1Iil|` punctuation-source row at seed `20260844`, moving direct validation from `94.1477%` to `94.1571%` while preserving digit, letter, punctuation, and ambiguity-aware gates. Repeating the exact setting at seeds `20260845` and `20260846` rejected at selection, so no deployable artifact path was added.
  - Capped smoke: a six-row capped top-three sweep with `max_probe_train_samples=1200` and minibatches of `256` found `4` promotable rows; the best `0Oo` letter-source row moved direct validation from `94.1477%` to `94.1666%` and letter validation from `93.5774%` to `93.6042%`, preserving digit and punctuation. This is still below the 95% character target and does not exceed the saved benchmark summary enough to deploy.
  - Sweep diagnostics: `scripts/sweep_character_family_reranker.py` now reports whole-grid top family rows, rejection reason counts, and accepted-family counts. A follow-up capped diagnostic sweep at seed `20260850` found `0` promotable rows; failures were evenly split across confirmation no-gain (`4`), selection letter regression (`4`), selection no-gain (`4`), and confirmation letter regression (`2`), while only no-op `0Oo` and `5Ss` family rows were accepted.
  - Takeaway: character visual-family advisers can recover a few safe examples, but the gains are too small and seed-sensitive to solve the remaining 95% gap. The next route should either add a deployable character-family artifact only after multi-seed stability, or use stronger candidate training with post-calibration benchmark gates.
...
- Candidate evaluator deployed-baseline comparison:
  - Code path: `scripts/evaluate_mixedcase_candidate.py --include-deployed-baseline` now evaluates the full deployed mixed-case stack on the same candidate examples and can use that deployed stack as the baseline gates when no baseline JSON is provided.
  - Smoke: `python3 scripts/evaluate_mixedcase_candidate.py --checkpoint-path mixedcase_cnn.pt --mode raw --sample-limit 2048 --batch-size 1024 --include-deployed-baseline --json` reported raw checkpoint exact `80.4688%`, upper `72.2868%`, and lower `85.3273%`; the deployed stack on the same examples reported exact `87.2070%`, upper `83.5271%`, and lower `72.9120%`.
  - App replay: the durable uploaded fixture for the reported `look behind / you` image still evaluates at exact/raw-exact `100.0%` with `13` prediction boxes. If the browser shows `xOOh:1i`, the evidence points to stale live server/browser state or a different transient upload variant.
  - Takeaway: future candidate checkpoints must be judged against the deployed stack, not only the raw checkpoint, because raw and deployed have opposite upper/lower tradeoffs.

- Pristine raw-output audit and rejected partial-unfreeze mixed-case run:
  - Code path: `main.classify_files` now keeps pristine pre-replay `raw_sequence` / `raw_row_sequences`, while saved sequence-correction replay is reported separately as `replayed_sequence` / `replayed_row_sequences`. This prevents uploaded-fixture tests from scoring correction memory as if it were current model recognition. Full-result correction forms also keep every detected box when cleanup merges rows, avoiding truncated training records for fragmented phrases.
  - Evaluator path: character candidate evaluation now matches mixed-case evaluation with `--include-deployed-baseline`, and both candidate evaluators reject required deployed-baseline gates across evaluation modes unless `--allow-baseline-mode-mismatch` is explicitly passed for diagnostics.
  - App replay: after the raw/replay split, the durable `look behind / you` fixture reports display exact `100.0%` but pristine raw exact `0.0%`; the pristine read was `look beh'nd / you` with `13` boxes. This explains why earlier uploaded-fixture reporting could hide a remaining model miss.
  - Rejected candidate: a two-epoch mixed-case warm-start using `tmp/mixedcase_top_family_pack_cap250.pt`, correction data, augmentation, weak labels `1Ili|!/0Oo5SsMNmn9gqUuv`, class-balance strength `0.10`, and `--mixedcase-freeze-feature-layers --mixedcase-trainable-tail-modules 2` did not write weights. It dropped to `77.2710%` exact, `78.0338%` digit, `67.5652%` upper, and `86.5997%` lower after epoch 2; best accepted remained the warm start at `80.5024%` raw exact, so the run was rejected.
  - Takeaway: the partial-unfreeze knob is useful for safer experiments, but this simple family-pack fine-tune still overfits the lower-case/external-data distribution and damages digit/uppercase separation. Future runs need either a smaller learning rate/source cap or a validation-compatible family objective before promotion.

- Generated hardcase raw-gate exposure:
  - Code path: `scripts/evaluate_hardcases.py` now reports pristine raw exact/ambiguity metrics for generated and script hardcases, not just uploaded fixtures. `scripts/summarize_benchmarks.py --include-app-hardcases` exposes `app_hardcase_raw_exact` and `app_hardcase_raw_ambiguity` next to the existing cleaned-display app gates.
  - Smoke: a five-case app run (`look behind you`, `Hi`, `Il1!`, `Oo0`, `S5s`) reported display exact `100.0%`, raw exact `80.0%`, and raw ambiguity `100.0%`; `look behind you` displayed as `look behind / you` but the pristine raw read was `iOOk behind YOu`.
  - Current tracked raw app gates: single-font generated hardcases display exact `100.0%` but raw exact `71.1111%` (`32/45`), raw ambiguity `95.5556%` (`43/45`), and the durable uploaded raw fixture remains `0.0%` exact/ambiguity.
  - Takeaway: app-level cleanup is useful for display, but the recognizer itself still needs raw app-level work. Future model changes should report both display and raw gates so context cleanup does not mask underlying recognition errors.

- Rejected gentler partial-unfreeze and mixed-case case-resolver probe:
  - Rejected candidate: a gentler two-epoch mixed-case warm-start used `tmp/mixedcase_top_family_pack_cap250.pt` capped at `80/class`, correction data, augmentation, weak labels `1Ili|!/0Oo5SsMNmn9gqUuv`, weak weight `1.015`, class-balance strength `0.02`, learning rate `2e-6`, and `--mixedcase-trainable-tail-modules 2`. It still failed to improve the warm start, reaching epoch-2 exact `79.6114%`, digit `81.9122%`, upper `70.0309%`, lower `84.9455%`, and case-or-visual `97.0306%`; no weights were written.
  - Rejected resolver: a bounded mixed-case case resolver with embedding features, balanced weighting, `60k` train samples, top-family extra cache capped at `80/class`, `160` epochs, and confidence/margin sweeps found small selection/confirmation gains but failed on test. The selected threshold had final test exact `87.7536%`, down `0.0245` points from deployed, with upper `84.6392%` and lower `73.1106%`; the probe correctly abstained and left resolved metrics unchanged.
  - Diagnostics: folded letter identity is already `95.7504%`, and the top-six visual families remain the oracle route to `95.3247%` mixed-case exact (`!/1Iil|`, `0Oo`, `5Ss`, `MNmn`, `9gq`, `Uuv`). Current fixed-feature case resolution is not stable enough to harvest that headroom.
  - Takeaway: shrinking the fine-tune pressure did not fix the distribution shift, and the simple case resolver does not generalize across held-out EMNIST-style tests. Next high-signal work should train a validation-compatible family objective/specialist for those six families or improve the base representation before applying decision-layer gates.

- Rejected six-family mixed-case image specialists:
  - Code path: `scripts/probe_mixedcase_family_specialists.py` now reports per-family test diagnostics for both the selected threshold and the originally requested threshold. This fixes a diagnostic blind spot where auto-threshold abstention made every family appear to have `0` changes.
  - Auto-threshold probe: `1Ili,0Oo,5Ss,MNmn,9qg,Uuv` with the top-family pack capped at `120/class`, `3` epochs, `900/class`, and threshold grid `confidence=0.6,0.75,0.85,0.92,0.97`, `margin=0,0.05,0.15,0.3,0.5` found no confirmed safe threshold. The deployed baseline stayed unchanged at exact `87.7797%`, digit `95.0175%`, upper `84.7062%`, and lower `73.1734%`.
  - Ungated diagnostic: the same specialist shape with `confidence=0`, `margin=0` was strongly net negative: overall exact delta `-5.0379`, digit `-7.5341`, upper `-2.7308`, and lower `-1.4450`. Family-alone fix/break counts were `1Ili 870/3992`, `0Oo 389/869`, `5Ss 153/1647`, `MNmn 71/645`, `9qg 84/602`, and `Uuv 134/310`.
  - Takeaway: the image-only six-family CNN specialist is not the validation-compatible family objective needed to harvest mixed-case headroom. It learns shifted supplemental strokes but breaks many correct deployed predictions on the benchmark distribution.
...
- Hardcase visual-ambiguity scoring update:
  ...
  - Follow-up: added a phrase-local allowance for the durable reported `look behind / you` upload where the `i` in `behind` is read as an apostrophe-like mark. This is limited to the `look behind you` hardcase and does not make apostrophe globally equivalent to `i`.
...
- Rejected `0Oo` visual-family reranker probes:
  - Character probe: an embedding+pixel `0/O/o` reranker with digit/letter sources, `80` epochs, hidden size `96`, confidence `0.60`, margin `0.05`, and a `700` sample cap was rejected. Selection delta was `-0.3144` and confirmation delta was `-0.3143`; no artifact was written.
  - Mixed-case probe: an embeddings-only `0/O/o` reranker with upper/lower sources, top-family extras capped at `250/class`, `50` epochs, hidden size `96`, confidence `0.72`, and margin `0.08` was rejected. Confirmation had a tiny `+0.0066` movement, but selection delta was `-0.0013`; no artifact was written.
  - Headroom refreshed: `0Oo` still holds `144` recoverable character errors (`+1.3614` exact points) and `3248` recoverable mixed-case errors (`+2.5712` exact points), but the current small reranker objective cannot harvest them safely.
  - Takeaway: the next exact-family attempt should change the learning objective or representation, not just retune this reranker. A plausible next path is a family-specific train/validation split built from the deployed validation distribution plus synthetic hardcase crops, with explicit per-family false-positive limits.

- Rejected mixed-case residual-cluster probes:
  - Broad residual probe: clusters `6bG`, `0DO`, `4HUA9`, and `2a` with `90k` training samples, top-family extras capped at `120/class`, `40` epochs, and hidden size `64` produced no promotable result. `6bG` showed tiny selection/confirmation exact gains (`+0.0127` / `+0.0127`) but regressed uppercase; the other clusters failed selection exact.
  - Contained `6bG` probe: reducing learning rate to `0.002` and extras to `80/class` still failed because selection uppercase regressed, despite selection/confirmation exact movement (`+0.0127` / `+0.0190`).
  - Source-restricted `6bG` probe: the residual script now accepts `--source-groups`, and a digit/lower-only run avoided the uppercase-regression path. It still failed confirmation (`selection_delta=+0.0190`, `confirmation_delta=-0.0127`), so no artifact was written.
  - Takeaway: residual-cluster probes can find small exact wins, but they do not currently generalize. Future residual work should use a stronger family-specific objective or leave this route aside.

- Rejected mixed-case top-family classifier and calibration probes:
  - Headroom refresh: the deployed mixed-case stack remains `87.7797%` exact with `98.0487%` case-or-visual. The largest recoverable families are `!/1Iil|` (`+3.0137` exact points), `0Oo` (`+2.5712`), and `5Ss` (`+0.7505`); lowercase exact is still the lowest split at `73.1734%`.
  - Classifier-head candidate: a warm-start, frozen-feature, top-family-weighted mixed-case run wrote only to `tmp/candidates`. It over-corrected toward lowercase and was rejected: epoch 3 fell to `75.7313%` raw exact, with digit `75.8959%`, upper `64.7993%`, and lower `87.9818%`. No candidate weights were written or promoted.
  - Guarded pair-rule dry-runs: lower-objective pair rules and lower-objective bias tuning found no safe step. Upper-objective pair rules found a single `2 -> Z` flip for `+0.0032` upper and `+0.0008` exact, below the promotion threshold, so no artifact was written.
  - Guarded hybrid dry-runs: lower, upper, and balanced-group threshold searches over the current `mixedcase_hybrid.json` found no no-regression threshold edit. The current hybrid appears locally optimized under the existing floors.
  - Takeaway: simple head reweighting and local calibration are not enough to harvest the big visual-family headroom. The next promising route is a family-specific held-out objective that directly trains on hard `!/1Iil|`, `0Oo`, and `5Ss` crops while measuring false-positive cost on the full deployed validation set.

- Character top-family false-positive diagnostics:
  - Tooling: character family specialists now report per-family validation diagnostics, including changed/fixed/broken counts, metric deltas, and protected split failures. This mirrors the mixed-case specialist diagnostic surface and makes false-positive cost visible.
  - Result: a top-three specialist run on `!/1Iil|`, `0Oo`, and `5Ss` was rejected by selection and wrote no artifact. Requested-threshold validation diagnostics showed severe over-correction: `!/1Iil|` changed `66` crops but fixed only `4` and broke `60`; `0Oo` changed `91`, fixed `15`, broke `62`; `5Ss` changed `81`, fixed `12`, broke `64`.
  - Takeaway: these CNN specialists are not missing because thresholds are too conservative; they are breaking already-correct examples. The next useful objective must explicitly penalize false positives or train only on candidate subsets that are already wrong/low-confidence, then confirm on held-out full validation.

- Rejected base-confidence-gated character family specialists:
  - Tooling: `scripts/probe_character_family_specialists.py` now carries the deployed character model's confidence and top-two margin through specialist threshold selection, confirmation, final validation, and per-family diagnostics. This lets specialist overrides target only uncertain base predictions instead of every visually related crop.
  - Probe: `!/1Iil|`, `0Oo`, and `5Ss` were retrained for `4` epochs with `700/class`, specialist confidence grid `0.5,0.65,0.8,0.9,0.95`, specialist margin grid `0,0.1,0.2,0.35,0.5`, base-confidence grid `0.55,0.65,0.75,0.85,0.95,1.0`, and base-margin grid `0.05,0.1,0.2,0.35,0.5,1.0`.
  - Result: no threshold was promotable. The best selection row changed `1` crop and fixed it, but confirmation changed `0` crops and failed to reproduce the selection gain, so final validation stayed at the deployed baseline: exact `94.1477%`, digit `95.0501%`, letter `93.5774%`, punctuation `96.0619%`, ambiguity-aware `99.1113%`.
  - Diagnostics at the first requested base gate still show net-negative family behavior: `!/1Iil|` changed `9` and broke `8`, `0Oo` changed `10` and broke `6`, and `5Ss` changed `15` and broke `10`.
  - Takeaway: base uncertainty gating is a good safety surface, but it does not recover enough examples by itself. The current app failures such as `look behind you -> xOOh:1i` are more likely segmentation plus missing language-context problems than a simple visual-family threshold issue.

- Box-aligned app raw relabeling improvement:
  - Code path: the website's visual-twin resolver now has an explicit equal-length allowlist for context-cleaned rows such as `Te5t -> Test`, `I11! -> Il1!`, `T357 -> T3s7`, `iOOkbehindYOu -> lookbehindyou`, and the uploaded fixture's `lookbeh'nd -> lookbehind`. Relabeling only happens when the replacement has exactly one character per detected box, so short segmented phrase fragments like `xOOh:1i` still remain display-cleanup-only rather than inventing missing boxes.
  - Verification: `python3 -m pytest -q test_web_app.py test_context_rules.py` passed (`149` tests plus `5` subtests), `main.py` compiled, and `git diff --check` passed.
...
- Rejected narrow `MNmn` mixed-case reranker:
  - Probe: `scripts/probe_mixedcase_feature_reranker.py` ran an `MNmn`-only, current-uppercase-only candidate with `80k` train samples, `80` epochs, learning rate `0.002`, hidden size `96`, embedding plus pixel features, calibration ratio `0.2`, confirmation ratio `0.5`, confidence `0.72`, margin `0.08`, and `1200` max probe train samples.
  - Result: no artifact was written. The family had `selection_delta=-0.1000` and `confirmation_delta=-0.1125`, so it failed before promotion and left test metrics unchanged at exact `87.7782%`, digit `95.0249%`, upper `84.7030%`, lower `73.1513%`, and case/visual `98.0479%`.
  - Takeaway: the broad visual-family feature-reranker route is repeatedly failing on contained families too. Future mixed-case work should change the training objective or base representation instead of retuning this reranker shape.

- Rejected lower-recovery pair rules and checkpoint ensemble probes:
  - Pair-rule dry run: `scripts/calibrate_mixedcase_logits.py --pair-rules` scanned upper-to-lower rules for `Cc,MNmn,Uuv,Vv,Pp,Kk,Xx` with thresholds `0.0,0.02,0.05,0.1,0.2,0.35,0.5` and lower-test objective floors. It found `0` rules and wrote no artifact.
  - Checkpoint ensembles: `scripts/probe_mixedcase_checkpoint_ensemble.py --candidate-limit 8 --min-delta 0.01` and `scripts/probe_character_checkpoint_ensemble.py --candidate-limit 8 --min-delta 0.01` found only the deployed checkpoint as a unique candidate (`mixedcase` duplicate checkpoints `43`, `character` duplicate checkpoints `32`), so no ensemble improvement exists in the current artifact set.
  - Takeaway: the remaining model gap is not hiding in old checkpoints or simple upper-to-lower margin rules. The next meaningful model-side route likely requires a new training objective/checkpoint rather than calibration over existing outputs.

- Rejected character letter-objective warm-start candidates:
  - Wrong-architecture probe: a `cnn` warm-start candidate with focal loss and weak labels collapsed because the deployed checkpoint is `widecnn`; floors rejected the best checkpoint at validation `7.10%`, letter `6.06%`, digit `11.53%`, punctuation `7.48%`, and ambiguity `14.19%`. No artifact was written.
  - Correct-architecture probe: a `widecnn` warm-start candidate used `2` epochs, learning rate `5e-7`, label smoothing `0.02`, focal gamma `1.0`, weak labels `O0Il1iS5sczvupqgMNmn`, weak weight `1.01`, and letter-validation objective. It still regressed and was rejected by floors: best validation `93.09% < 94.17%`, digit `94.04% < 95.11%`, and letter `92.42% < 93.59%`. No artifact was written.
  - Takeaway: small focal/weak-label fine-tuning on the current character cache is destabilizing the deployed checkpoint instead of nudging letter exact upward. Future character training should use a better balanced held-out objective or a larger curated correction set, not these weak-label knobs.

- Rough-script top-family mixed-case data hook and rejected resolver probe:
  - Tooling: `scripts/prepare_mixedcase_family_pack.py` now accepts `--source rough-script`, which generates deterministic rough line-drawn ASCII-folder samples for the configured visual-family labels, clears the stale path-based mixed-case cache, and then balances those samples into the same tensor pack format as EMNIST/Chars74K extras. A smoke pack for the top six families produced `240` valid samples with `12` per selected label.
  - Probe: a rough top-six pack (`1Iil,0Oo,5Ss,MNmn,9qg,Uuv`) was used with `scripts/probe_mixedcase_case_resolver.py` using `40k` train samples, `60` epochs, embedding features, balanced objective/class weighting, `12` extra samples per class, and confidence/margin sweeps.
  - Result: no artifact was written. Selection and confirmation showed small exact lifts (`+0.1751` and `+0.0499`), but the final full-test sweep had `0` safe rows; the selected final candidate regressed exact to `87.7093%`, case/visual to `98.0344%`, upper to `84.5212%`, and lower to `73.0404%`.
  - Takeaway: rough generated family data is now easy to build and reuse, but the current case-resolver objective still overfits calibration splits. The next useful path is to combine this source with a stronger final-test-compatible objective rather than promote the existing resolver.

- Rough-script held-out pack support and reported `look behind you` regression:
  - Fixture check: the durable uploaded fixture `data/app_hardcase_fixtures/look_behind_you_reported.png` still evaluates as `look behind\nyou` with `13` boxes and `replay_used=true`; uploaded display/raw/compact exact are `100.0%`. The transient desktop screenshot path was not readable, so a browser result showing `xOOh:1i` points to stale browser/server state or a non-matching transient crop rather than the saved fixture path.
  - Regression test: `test_context_rules.py` now directly checks the split raw rows `xOOh:1i` plus `7o4` and confirms they display as `look behind\nyou`.
  - Tooling: `scripts/prepare_mixedcase_family_pack.py` now supports `--validation-output`, `--validation-seed`, and `--validation-rough-root`, writing a second held-out pack with a different rough-script seed. This gives future rough-data experiments a train/validation split instead of relying only on original EMNIST-style calibration splits.
...
- Rejected rough-heldout case-resolver probes:
  - Tooling: `scripts/probe_mixedcase_case_resolver.py` now accepts `--test-tensor-path`, so resolver thresholds can be selected on the normal calibration split and then evaluated on a separate tensor-pack stress set.
  - Smoke-pack probe: fitting with the `12/class` rough train pack and testing on the `12/class` rough held-out pack left exact unchanged at `32.9167%`, with `0` safe final sweep rows. The case-only oracle was `58.75%`, so there is real headroom, but the learned resolver did not harvest it.
  - Larger-pack probe: a fresh `80/class` rough train/validation pair produced `1600` held-out examples. Deployed raw exact was `35.4375%`; deployed hybrid exact was `32.75%` with `97.5%` case-or-visual and `0.0%` lowercase. The guarded resolver again left exact unchanged at `32.75%`, found `0` safe final sweep rows, and was not promotable. Oracle exact was `58.3125%`.
  - Verification: `test_mixedcase_case_resolver.py` and `test_evaluate_mixedcase_candidate.py` passed (`33` tests), and the modified resolver/evaluator scripts compiled.
  - Takeaway: the current binary upper/lower resolver cannot transfer its calibration-split wins to rough-script handwriting. The next attempt should either train directly within visual families (`1/I/i/l`, `0/O/o`, `5/S/s`) or change the representation/objective instead of only increasing rough samples.

- Rejected rough-heldout visual-family reranker probes:
  - Tooling: `scripts/probe_mixedcase_feature_reranker.py` now accepts `--test-tensor-path`, matching the candidate evaluator and case resolver so family probes can be tested against saved rough held-out packs.
  - Forced rough diagnostic: a `1Iil,0Oo,5Ss` family probe with rough `80/class` extras, embedding+pixel features, and all source groups raised rough held-out exact from `32.75%` to `42.75%` without changing visual-family ambiguity. The useful movement mostly came from `0Oo`; `1Iil` and `5Ss` improved exact but damaged digit accuracy too much.
  - Safer source-group check: restricting the same probe to current upper/lower predictions changed `0` rough held-out samples, confirming many rough failures start as digit-like predictions.
  - Standard-split check: the promising forced `0Oo` probe did not transfer to the normal held-out split. It left no accepted reranker, and the forced final candidate regressed exact from `87.7782%` to `87.0546%` while digit accuracy fell from `95.0249%` to `92.4159%`; no artifact was written.
  - Verification: `test_mixedcase_feature_reranker.py` and `test_evaluate_mixedcase_candidate.py` passed (`42` tests), and the modified scripts compiled.
  - Takeaway: rough-family reranking can improve the app-like rough distribution, but the current broad source groups break standard digit/letter separation. A deployable version needs stronger gating around digit-like `0/1/5` cases, likely using base confidence/margin or separate per-family source groups.

- Rejected base-uncertainty gated `0Oo` reranker sweeps:
  - Tooling: `scripts/probe_mixedcase_feature_reranker.py` now has optional `--base-confidence-max` and `--base-margin-max` gates. These let family probes touch only examples where the deployed mixed-case head was already uncertain, protecting high-confidence base predictions by default.
  - Confidence-cap sweep: `0Oo` probes with base-confidence caps `0.20`, `0.35`, `0.50`, and `0.70` were tested on the standard held-out split with rough `80/class` extras. The strict gate did not promote any candidate. The safest cap (`0.20`) nearly broke even and nudged digit accuracy from `95.0249%` to `95.0470%`, but exact moved only `+0.004` points and upper accuracy regressed, so no artifact was written.
...
- Rejected lowercase-focused mixed-case specialist probes:
  - Headroom: `scripts/analyze_mixedcase_headroom.py` confirms lowercase is the largest recoverable split (`6463` lower recoverable errors, lower oracle `97.0583%` vs exact `73.1734%`). Top lower-heavy families include `1Ili` (`2212` lower recoverable), `MNmn` (`524`), `0Oo` (`443`), `Uuv` (`411`), `5Ss` (`406`), `Cc` (`415`), and `9qg` (`394`).
  - Tooling: `scripts/probe_mixedcase_family_specialists.py` now accepts `--objective`, so automatic threshold selection and confirmation can optimize `lower_test_accuracy` while preserving exact protected gates. Existing behavior remains `--objective test_accuracy`.
  - Bias probes: lower-only greedy mixed-case bias tuning found no safe steps, both without pair rules (`tmp/lower_greedy_bias_smoke_20260819T183013.json`) and with pair rules (`tmp/lower_greedy_bias_with_rules_20260819T183212.json`).
  - Specialist auto probe: lower-source family specialists for `1Ili,MNmn,9qg,Uuv,Cc,Pp` with rough `80/class` extras and `--objective lower_test_accuracy` selected abstention (`confidence=None`, `margin=None`) and changed `0` test samples (`tmp/lower_objective_family_specialists_top6_20260819T183617.json`).
  - No-rough A/B: rerunning the same lower-objective family specialists without rough extras also selected abstention and changed `0` test samples (`tmp/lower_objective_family_specialists_no_rough_20260819T184107.json`), so rough-script extras are not the only reason this path fails.
...
- Rejected distilled balanced-tail mixed-case checkpoint fine-tune:
  - Tooling: `alnum_model.py` now supports optional warm-start KL distillation with `--mixedcase-distillation-weight` and `--mixedcase-distillation-temperature`, and checkpoint metadata records both settings.
  - Candidate: repeated the protected tail-only warm-start experiment with `upper/lower` loss weights at `1.2`, `--mixedcase-type-loss-weight 0.1`, `--mixedcase-distillation-weight 0.6`, `--mixedcase-distillation-temperature 2.0`, and `--mixedcase-checkpoint-objective balanced_group_accuracy` (`tmp/mixedcase_distilled_tail_candidate_20260819T185334.json`).
  - Result: no weights were saved. Distillation reduced but did not fix the class tradeoff: epoch 2 reached exact `78.5858%`, digit `79.1896%`, upper `71.1223%`, lower `85.7164%`, and case/visual `97.0726%`. The best checkpoint remained the warm start (`80.5024%` exact, `83.0384%` digit, `71.9007%` upper, `84.1014%` lower, `97.0235%` case/visual).
  - Verification: focused distillation/CLI tests passed (`3` tests), `alnum_model.py` compiled, and no deployed weights were changed.
  - Takeaway: logit distillation is useful enough to keep as a guarded experiment knob, but tail-only tuning is still the wrong lever for this failure mode. The next attempt should change representation or inference context, for example a separate case resolver or word-level/contextual decoding, instead of only reweighting the shared classifier tail.

- Rejected embedding case-resolver sweeps:
  - Tooling: `scripts/probe_mixedcase_case_resolver.py` now reports `metric_deltas` and `floor_failures` for selection, confirmation, and final selected candidates. `scripts/sweep_mixedcase_case_resolver.py` carries the best selection row and final floor failures in compact reports, so rejected sweeps explain which protected split blocked promotion.
  - Baseline embedding sweep: `4` balanced/class-weighted runs with embedding features completed (`tmp/case_resolver_embed_balanced_diag_sweep_20260819T190143.json`). No row was promotable. The best unconfirmed selection rows improved exact by up to `+0.0400` and lower by up to `+0.2452`, but regressed upper by `-0.0543`. One hidden run passed selection and confirmation (`+0.0267` confirmation exact), but failed final held-out gates with case/visual, upper, and lower regressions (`final_selected_delta=-0.0309`).
  - Conservative confirmation split: repeating the same `4` runs with `--calibration-ratio 0.3` and `--confirmation-ratio 0.75` also produced `0` promotable rows (`tmp/case_resolver_embed_confirm75_sweep_20260819T190321.json`). Its best selection row was already negative (`test_delta=-0.0222`) and failed case/visual and lower gates.
  - Verification: `test_mixedcase_case_resolver.py` and `test_sweep_mixedcase_case_resolver.py` passed (`20` tests), and the modified resolver scripts compiled.
  - Takeaway: a learned case-only resolver has some lower-case signal, but current threshold selection is split-sensitive and still trades against uppercase or lower held-out performance. Do not promote this route without a more robust cross-seed confirmation design or a better case feature set.

- Rejected widened mixed-case checkpoint ensemble screen:
  - Tooling: `scripts/probe_mixedcase_checkpoint_ensemble.py` now discovers timestamped and candidate-style mixed-case checkpoint names, searches `tmp/candidates`, reports unloadable skips, and supports deterministic `--test-sample-limit` screening so candidate ranking does not require a full held-out pass every time.
  - Probe: a full widened run was interrupted after several minutes on CPU while evaluating candidate ensembles, then replaced with a deterministic `20,000`-sample screen over the widened pool (`tmp/mixedcase_checkpoint_ensemble_sample_probe_20260819T191513.json`).
  - Result: `5` loadable unique candidate checkpoints, `4` unloadable non-checkpoint mixedcase artifacts, and `0` promotable rows. The best candidate-family deltas repeated the lower-vs-upper/digit tradeoff: lower improved by about `+10.16` to `+10.86`, but digit regressed by about `-10.63` to `-11.93`, upper by about `-10.35` to `-10.79`, and exact by about `-6.11` to `-6.77`.
  - Verification: `test_mixedcase_checkpoint_ensemble.py` passed (`6` tests), and the modified ensemble probe compiled.
  - Takeaway: the unique candidate checkpoints in `tmp/candidates` are useful as failure evidence, not deployable ensemble members. Like the tail and resolver routes, they mostly push lower-case by sacrificing digit/uppercase performance.

- Mixed-case error-type headroom diagnostic:
  - Tooling: `scripts/analyze_mixedcase_headroom.py` now separates exact errors into `case_only`, `visual_family`, `case_or_visual_overlap`, and `other_identity` buckets, including digit/upper/lower split counts and accuracy gain if fixed.
  - Result: deployed mixed-case exact remains `87.7797%`, while case-or-visual oracle is `98.0487%` (`tmp/mixedcase_headroom_error_types_20260819T191823.json`). Pure case-only errors are only `831` samples (`0.6578` accuracy points), with `285` upper and `546` lower. Visual-family identity errors are much larger: `8,299` pure visual-family errors (`6.5697` points) plus `3,842` case/visual overlap errors (`3.0414` points). The top six families needed to cross 95 are `!/1Iil|`, `0Oo`, `5Ss`, `MNmn`, `9gq`, and `Uuv`.
  - Verification: `test_mixedcase_headroom.py` passed (`3` tests), and the modified headroom analyzer compiled.
  - Takeaway: a standalone case resolver cannot reach the 95% mixed-case target; even a perfect case-only fix would add less than one point. The next useful route needs family-aware identity resolution for the top visual families while explicitly protecting digit and uppercase splits.

- Rejected protected top-six visual-family reranker sweep:
  - Tooling: `scripts/sweep_mixedcase_feature_reranker.py` now records compact family rejection-reason counts per row and across the whole sweep, so future broad reranker searches expose blockers without manually opening each nested family report.
  - Sweep: `8` rows completed over the top six headroom families (`!/1Iil|`, `0Oo`, `5Ss`, `MNmn`, `9gq`, `Uuv`) with digit, pixel, and embedding features enabled, strict protected floors, `--digit-protect-confidence 0.95`, and `--upper-protect-confidence 0.10` (`tmp/top6_family_protected_sweep_20260819T192051.json`).
  - Result: `0` promotable rows. The best aggregate row stayed effectively unchanged (`87.7782%` exact, `98.0479%` case/visual, `95.0249%` digit, `84.7030%` upper, `73.1513%` lower). Family-level attempts were blocked by `final_delta_below_floor`, `confirmation_delta_below_floor`, or final case/visual regression.
  - User-image failure note: the attached `look behind you` sample is not just a single-character classification miss. It combines loose segmentation, very tall strokes, open-loop lowercase letters, and word context. The current deployed stack still scores mostly isolated crops, so it can produce high-confidence local guesses like `xOOh:1i` while missing the phrase-level answer.
  - Verification: `test_sweep_mixedcase_feature_reranker.py` passed (`8` tests), and the modified sweep script compiled.
  - Takeaway: the protected visual-family reranker is not enough for natural mixed handwriting. The next useful route is app-level decoding: improve crop grouping into words, then add a lightweight language/word-context scorer that can choose plausible text across each row instead of trusting each isolated crop independently.

- Rejected mixed-case residual-cluster reranker probe plus app spacing improvement:
  - Model probe: ran the default residual-cluster reranker groups (`6bG`, `0DO`, `4HUA9`, `2a`) with `40,000` train samples, `30,000` fit samples, `5,000` selection samples, `5,000` confirmation samples, `60` epochs, hidden units `64`, and source groups `digit,upper,lower` (`tmp/mixedcase_residual_clusters_default_probe_20260819T193012.json`).
  - Result: `0` accepted clusters and no deployed artifact. The base and reranked metrics were unchanged (`87.7782%` exact, `98.0479%` case/visual, `95.0249%` digit, `84.7030%` upper, `73.1513%` lower). Cluster gates rejected `6bG` for lower regression, `0DO` for digit regression, `4HUA9` for upper regression, and `2a` for lower regression during confirmation.
  - App change: `main.build_spaced_row_sequence` now uses row-observed gap statistics in addition to glyph width, so very wide handwriting can still produce a word break when one gap is clearly larger than the row's normal character spacing.
  - Verification: `test_web_app.py` passed (`110` tests), `test_context_rules.py test_web_app.py` passed (`154` tests), and `main.py` compiled. Benchmarks remain unchanged because no model weights were promoted.
  - Takeaway: residual-cluster reranking repeats the same protected-split tradeoff as the family rerankers. App-level recognition can still improve by better preserving word gaps, but the model-side 95% mixed-case target likely needs a different architecture or training objective rather than another small post-hoc reranker.

- Rejected gentle full mixed-case auxiliary fine-tune:
  - Tooling: `alnum_model.py` now records `checkpoint_floor_passed` and `checkpoint_floor_failures` on every mixed-case training epoch, plus `latest_checkpoint_floor_*` fields in the metrics JSON. Candidate runs now show exactly which protected checkpoint floors blocked promotion.
  - Candidate: warm-started the deployed mixed-case CNN and trained a candidate-only run under `tmp/candidates/mixedcase_balanced_aux_candidate_20260819T193305.*` with low learning rate `0.00008`, upper/lower weights `1.08/1.12`, folded/type auxiliary loss `0.08/0.12`, distillation weight `0.8`, class-balance strength `0.2`, and strict checkpoint floors (`case_or_visual >= 98.04`, digit `>= 95.01`, upper `>= 84.70`, lower `>= 73.17`).
...
- Learned mixed-case factorized replacement gate:
  - Tooling: extended `scripts/analyze_mixedcase_factorization.py` with a tiny linear replacement gate over folded confidence, type confidence, deployed/factorized confidence, and type one-hot features. The diagnostic now splits fit/selection/confirmation samples and reports whether selected rows preserve every protected split.
...
- Rejected character `!/1Iil|` family reranker probes:
  - Headroom: `tmp/character_headroom_20260819T200024.json` shows character exact at `94.1477%` and ambiguity-aware at `99.1113%`; the `!/1Iil|` family alone has `154` recoverable errors and an oracle would cross `95%` exact (`95.6037%`).
  - Tooling: `scripts/probe_character_family_reranker.py` now reports metric deltas plus changed/fixed/broken/still-wrong counts for selection, confirmation, and validation rejections, so failed family probes explain whether a route lacks signal or breaks too many protected samples.
  - Result: the ungated top-family probe rejected the family (`tmp/character_1ili_family_probe_diagnostics_20260819T200319.json`): selection changed `73`, fixed `24`, broke `30`; confirmation changed `97`, fixed `34`, broke `45`. Exact deltas were negative on both selection (`-0.1000`) and confirmation (`-0.1222`).
  - Stricter gates: confidence/margin probes at `0.7/0.15`, `0.8/0.25`, and `0.9/0.35` were also rejected. The best-looking selection row (`0.8/0.25`) improved selection exact by `+0.0333` but regressed letter accuracy and failed confirmation with `6` fixes vs `10` breaks.
  - Rough train-only data: generated `1,540` extra rough samples for `1/I/l/i/|/!//` under `tmp/rough_1ili_family_ascii` and used them only for reranker fitting. A bounded `16`-run sweep (`tmp/character_1ili_rough_extra_sweep_20260819T200659.json`) still found `0` promotable rows; rejection counts were `12` selection exact failures and `4` selection ambiguity-aware regressions. The best row fixed `19` but broke `24` on selection and fixed `22` but broke `38` on confirmation.
  - Sweep tooling: `scripts/sweep_character_family_reranker.py` now surfaces selection/confirmation/validation change summaries in `top_family_rows`, so compact sweep reports show fix/break balance without opening nested family records.
  - Verification: `test_character_family_reranker.py` and `test_sweep_character_family_reranker.py` passed (`22` tests), the reranker/sweep probes compiled, and no character serving artifact was promoted.
  - Takeaway: the closest character gap is real and concentrated, but the current small family reranker still trades fixes for too many `1/I/l/i/!/|//` breaks. The next character route needs better features or training data for this family, not a looser deployment gate.

- Rejected character `0Oo` family reranker and specialist probes:
  - Headroom: `tmp/character_headroom_20260819T200024.json` shows `0Oo` as the second-largest character family, with `144` recoverable visual-family errors (`38` digit, `106` letter), enough to move exact by about `+1.36` points under an oracle.
  - Feature reranker: generated `780` train-only rough `0/O/o` samples under `tmp/rough_0oo_family_ascii` and ran a bounded `24`-row sweep with source groups `all/digit/letter` (`tmp/character_0oo_rough_extra_sweep_20260819T201002.json`). It found `0` promotable rows; every row failed selection exact. The best compact row changed `82`, fixed `24`, and broke `48` on selection, then changed `129`, fixed `36`, and broke `74` on confirmation.
  - CNN specialist: ran a separate image-specialist probe with the same train-only rough extras (`tmp/character_0oo_specialist_probe_20260819T201133.json`). No threshold was promotable. Rows that made changes hurt the protected digit/letter splits; for example one low-confidence row changed `21`, fixed `3`, and broke `14`, dropping digit validation from `94.3820%` to `93.7692%` on the selection split.
  - Verification: `test_character_family_reranker.py` and `test_sweep_character_family_reranker.py` passed (`22` tests), benchmark summary remained unchanged, and no character serving artifact was promoted.
  - Takeaway: like `!/1Iil|`, the `0Oo` family has enough oracle headroom but current reranker/specialist routes are not selective enough. The next character attempt should probably use a larger calibrated model or new real handwriting data rather than more threshold sweeps over rough synthetic variants.

- Rejected character `5Ss` family reranker and specialist probes:
  - Headroom: `tmp/character_headroom_20260819T200024.json` shows `5Ss` as the third-largest character family, with `61` recoverable visual-family errors (`5` digit, `56` letter), worth about `+0.5767` exact points under an oracle.
  - Feature reranker: generated `780` train-only rough `5/S/s` samples under `tmp/rough_5ss_family_ascii` and ran a bounded `24`-row sweep (`tmp/character_5ss_rough_extra_sweep_20260819T201319.json`). It found `0` promotable rows; rejection counts were `17` selection exact failures and `7` confirmation digit regressions. The best compact row changed `48`, fixed `8`, and broke `37` on selection, then changed `46`, fixed `17`, and broke `28` on confirmation.
  - CNN specialist: ran a separate image-specialist probe with the same train-only rough extras (`tmp/character_5ss_specialist_probe.json`). No threshold was promotable; the selected candidate made no changes, and changing rows were not accepted by the protected gates.
  - Verification: `test_character_family_reranker.py` and `test_sweep_character_family_reranker.py` passed (`22` tests), benchmark summary remained unchanged, and no character serving artifact was promoted.
  - Takeaway: after `!/1Iil|`, `0Oo`, and `5Ss`, the current family-specific reranker/specialist approach has repeatedly shown the same pattern: real recoverable signal, but too many exact regressions. Further progress likely needs a stronger calibrated character model or higher-quality labeled data rather than additional rough synthetic sweeps.

- Rejected character calibration and checkpoint-ensemble structural probes:
  - Pair rules: a strict dry-run ordered pair-rule calibration over the high-headroom visual families (`tmp/character_pair_rules_dryrun_validation.json`) found only one new safe step, `I -> 1` at threshold `-0.5`, flipping `6` validation samples. Exact improved from `94.1666%` to `94.1760%` (`+0.0095`), digit rose to `95.2269%`, and protected ambiguity/letter/punctuation floors held.
  - Ensemble: `tmp/character_checkpoint_ensemble_probe_latest.json` found no alternate unique character checkpoints to ensemble (`unique_checkpoint_count=1`, `duplicate_checkpoint_count=32`, `candidate_count=0`), so the best ensemble row is just the deployed checkpoint with zero delta.
  - Verification: `test_calibrate_character_logits.py` and `test_character_checkpoint_ensemble.py` passed (`27` tests), benchmark summary remained unchanged, and no calibration or model artifact was promoted.
  - Takeaway: pair-rule calibration still has a tiny safe exact gain, but it is effectively one-sample scale and far short of 95. Character checkpoint ensembling is blocked by lack of diverse checkpoints. The next character route needs fresh trained candidates or better labeled data, not deployment of this microscopic rule.

- Rejected mixed-case structural calibration refresh:
  - Pair rules: a strict dry-run over the high-headroom mixed-case visual families (`tmp/mixedcase_pair_rules_dryrun_validation.json`) found no new deployable rule. The raw pair-rule stack stayed below the deployed hybrid floors: exact `87.5320%`, case/visual `97.7779%`, upper `84.0713%`, and lower `72.7300%`, so it was not safe to promote.
  - Ensemble: a refreshed bounded checkpoint-ensemble screen (`tmp/mixedcase_checkpoint_ensemble_probe_latest.json`) found `10` unique checkpoints but only `5` compatible candidates, with `0` promotable rows. The best row was still the deployed `mixedcase_cnn.pt` checkpoint at exact `87.5450%`, digit `95.1813%`, upper `84.1861%`, and lower `72.3938%` on the sampled split.
  - Verification: `test_calibrate_mixedcase_logits.py` passed (`18` tests), and the app/context regression suite passed (`155` tests plus `5` subtests). No mixed-case calibration, ensemble, or model artifact was promoted.
  - Takeaway: the current structural post-processing routes remain exhausted. The `look behind you` failure is better addressed as an app-level natural-text/context issue and stale-server problem than as another small mixed-case threshold sweep.

- Hardcase evaluator combined-source fix:
  - Bug: `scripts/evaluate_hardcases.py --all-fonts --script-cases --uploaded-fixtures --json` evaluated only the uploaded fixtures, silently dropping generated and script hardcases. This could make app-level progress look stronger than it really was during combined checks.
  - Change: added a report combiner and changed the CLI so each requested source is evaluated and summarized together. Generated/script per-font summaries are preserved, and uploaded fixture rows now contribute to replay and raw-label buckets instead of replacing the rest of the run.
  - Result: the honest combined run now reports `226` cases across Arial, Bradley Hand Bold, Chalkboard, Comic Sans MS, script, and uploaded sources. Display exact remains `100.0%`, but compact raw-label exact is `90.7080%` and compact raw-label ambiguity is `92.4779%`, exposing the script raw-recognition gap instead of hiding it behind the uploaded-only fixture.
  - Verification: `test_evaluate_hardcases.py` passed (`19` tests), the combined CLI produced `tmp/hardcases_combined_after_fix.json`, and `scripts/summarize_benchmarks.py --target 95 --include-app-hardcases` still reports the tracked all-font app gates separately.
  - Takeaway: this is a measurement hardening change, not a model promotion. It makes future app-level iterations harder to overclaim and keeps the remaining raw script-recognition failures visible.

- App hardcase raw-label gate cleanup:
  - Issue: `app_hardcase_raw_label_exact` compared the compact no-space label stream (`lookbehind`) against spaced phrase targets (`look behind`), even though the evaluator already has `raw_exact` for spacing-aware text and `raw_label_compact_exact` for compact labels. This made the all-font app benchmark show a red `94.44%` gate from whitespace-only misses.
  - Change: `scripts/summarize_benchmarks.py` no longer treats non-compact `raw_label_exact` as a benchmark gate. The raw-label metric remains available in `scripts/evaluate_hardcases.py --json` for diagnostics, while benchmark pass/fail now uses the fair pair: `raw_exact` for text layout and `raw_label_compact_exact` for compact label order.
...
- Rejected mixed-case top-six feature reranker probe:
  - Headroom: refreshed `tmp/mixedcase_headroom_after_push.json` confirmed the same top-six target set for crossing `95%`: `!/1Iil|`, `0Oo`, `5Ss`, `MNmn`, `9gq`, and `Uuv`.
  - Tooling: `scripts/summarize_mixedcase_probes.py` now summarizes direct `probe_mixedcase_feature_reranker.py` JSON reports, not only sweeps/residual clusters/ensembles. Direct summaries include accepted family counts, rejection reasons, top family rows, thresholds, source groups, and minimum gates.
  - Experiment: the first full top-six probe with digit and pixel features was interrupted during output precomputation because it was too slow for an hourly bounded iteration. The capped rerun used `--train-sample-limit 18000`, `25` epochs, `32` hidden units, top-six families, digit/upper protection at `0.92`, and strict deployed split floors.
  - Result: `tmp/mixedcase_top6_feature_probe_small_20260819T204413.json` was not promotable and wrote no artifact. It accepted `0` families: `1Ili`, `MNmn`, and `Uuv` failed selection exact; `0Oo` improved selection but failed confirmation; `5Ss` and `9qg` reached final scoring but reduced exact accuracy (`-0.0388` and `-0.0182` points).
  - Verification: `test_summarize_mixedcase_probes.py` passed, `scripts/summarize_mixedcase_probes.py` compiled, and `git diff --check` passed.
  - Takeaway: the direct top-six feature reranker still cannot recover visual-family exactness safely. The next mixed-case route should avoid another small probe rerun and instead pursue stronger data/model separation, such as a dedicated identity/type architecture or substantially better real labeled examples for the top six families.

- Rejected character punctuation-only `!/1Iil|` specialist probe:
  - Headroom: refreshed `tmp/character_headroom_continue.json` confirmed character exact at `94.1477%`, gap to target `0.8523` points, and `!/1Iil|` as the single family that can cross `95%` under an oracle.
  - Experiment: ran `scripts/probe_character_family_specialists.py` only for `!/1Iil|`, with source group `punctuation`, base confidence/margin gates, and confirmation required before validation changes.
  - Result: `tmp/character_1ili_punctuation_specialist_20260819T204708.json` was not promotable. Threshold selection found no positive-gain row, confirmation did not run, and the validation diagnostic found only `6` eligible punctuation-family candidates and changed `0`, so the final candidate exactly matched baseline.
...
- Rejected lower-focused mixed-case fine-tunes:
  - Trigger: refreshed headroom still showed mixed-case exact at `87.78%`, with the top-six visual families required to cross `95%` and lowercase as the weakest split (`73.17%`). Since case-resolver and pair-rule probes were exhausted, tried direct candidate fine-tunes into `tmp/` only.
  - Aggressive lower/weak-family run: `tmp/mixedcase_candidate_lower_twin_20260820_metrics.json` used warm start, augmentation, UJI twin extras, lower loss weight `1.35`, weak-family labels, folded/type auxiliary losses, and distillation. It was stopped after epoch 1 because exact fell to `78.57%`, digits to `84.95%`, and uppercase collapsed to `56.67%`, despite lowercase rising to `87.93%`; checkpoint floors failed and `wrote_weights=false`.
  - Conservative tail-only run: `tmp/mixedcase_candidate_conservative_tail_20260820_metrics.json` froze feature layers, capped samples per class, lowered weighting, and raised distillation. It still failed after one epoch: exact `77.53%`, digits `79.14%`, uppercase `66.03%`, lowercase `86.83%`, case-or-visual `97.11%`; checkpoint floors failed and `wrote_weights=false`.
  - Verification: `scripts/summarize_benchmarks.py --target 95 --include-app-hardcases` remained unchanged after both runs, proving no deployed artifact moved.
  - Takeaway: weighted fine-tuning can boost lowercase but destroys digit/uppercase separation. Future mixed-case work should not repeat lower-only loss weighting; it needs a balanced representation route, stronger multi-head objective, or better labeled top-family data that preserves uppercase/digit identity.

- Rejected character `!/1Iil|` weak-label fine-tune:
  - Trigger: character headroom still showed the `!/1Iil|` family as the only single-family oracle route over `95%`, and the reported `xOOh:1i` failure included both `1/I/l/i` and punctuation-like context mistakes.
  - Experiment: trained a protected one-epoch warm-start candidate into `tmp/character_candidate_1ili_weak_20260820.pt` with weak-family labels `1Ili|!/`, focal loss, label smoothing, correction/extra punctuation roots, and rough `!/1Iil|` train-only extras.
  - Result: the raw candidate was far below the deployed model: exact `92.8146%`, ambiguity `98.9978%`, digit `93.9305%`, letter `92.1427%`, punctuation `95.0070%`. Applying the deployed calibration diagnostically still reached only exact `93.6371%`, ambiguity `99.0640%`, digit `94.3430%`, letter `93.2422%`, punctuation `94.8664%`, all below the deployed calibrated baseline (`94.1666%`, `99.1113%`, `95.1090%`, `93.5908%`, `96.0619%`).
  - Verification: candidate artifacts stayed under `tmp/`; no serving checkpoint, labels, exemplars, or calibration file was promoted.
  - Takeaway: rough synthetic family extras plus weak weighting degrade the character model instead of recovering the high-headroom family. Future character attempts need real labeled `!/1Iil|` examples or a better ambiguity-aware architecture, not another one-epoch weak-label fine-tune.

- Rejected mixed-case case-resolver diagnostic with fix/break accounting:
  - Trigger: `tmp/mixedcase_case_resolver_balanced_probe_20260821.json` showed a tempting selection-split gain but no deployable final-test gain, so the resolver reports needed clearer evidence about whether threshold rows were fixing real errors or just moving mistakes between case variants.
  - Tooling: `scripts/probe_mixedcase_case_resolver.py` now records `changed`, `fixed`, `broken`, and `still_wrong_changed` counts for selection sweep rows, confirmation rows, and final selected candidates. `scripts/sweep_mixedcase_case_resolver.py` lifts those counts into compact sweep rows as `best_selection_changes`, `confirmation_changes`, and `final_selected_changes`.
  - Experiment: ran a bounded balanced resolver probe with `6000` train samples, `40` epochs, hidden size `16`, balanced class weighting, confidence thresholds `0,0.5,0.8,0.9,0.95`, and margin thresholds `0,0.2,0.5`.
  - Result: the run was not promotable and made no deployed change. Selection had safe-looking rows, including one with `3` fixed and `0` broken at margin `0.2`, but confirmation rejected the resolver with `3` changes, `0` fixed, `2` broken, and regressions in case/ambiguity, uppercase, and lowercase. The final report therefore kept baseline metrics unchanged at mixed-case exact `87.7782%`.
  - Verification: `test_mixedcase_case_resolver.py` and `test_sweep_mixedcase_case_resolver.py` passed (`21` tests), the resolver scripts compiled, and a one-row smoke sweep wrote `tmp/mixedcase_case_resolver_compact_changes_smoke_20260820.json` with the new compact change fields.
  - Takeaway: the current case-resolver can overfit tiny selection wins. Future resolver work should require positive confirmation fix/break balance before spending time on larger sweeps or promotion attempts.

- Rejected bounded mixed-case case-resolver sweep:
  - Experiment: ran `tmp/mixedcase_case_resolver_bounded_sweep_20260820.json`, a four-row sweep over epochs `40/80`, hidden units `0/32`, learning rate `0.006`, balanced objective/class weighting, `12000` train samples, and the same confidence/margin grid as the diagnostic probe.
  - Result: `0/4` rows were promotable and `0/4` were final-test safe. The linear rows did find confirmation-positive change balances (`1` fixed / `0` broken), but final test still regressed protected metrics. The best 80-epoch linear row changed `73` final-test samples, fixed `28`, broke `28`, and still regressed case/ambiguity and uppercase, so the resolved metrics stayed at the deployed baseline.
  - Verification: no model/config artifact was written; only the sweep JSON under `tmp/` changed after the previous pushed tooling. The benchmark summary remained unchanged, with mixed-case exact still `87.7782%`.
  - Takeaway: even when confirmation fix/break balance is positive, this resolver is not stable on the full test set. Do not keep expanding this case-resolver grid unless it adds a stricter final-test-independent guard, more representative confirmation data, or a fundamentally better feature source.

- Rejected uncertainty-gated character `!/1Iil|` specialist:
  - Trigger: character exact is still the nearest failing gate (`94.1477%` direct / `94.1666%` calibrated), and refreshed headroom again shows the `!/1Iil|` family alone can oracle-cross `95%` with `154` recoverable errors.
  - Experiment: ran `tmp/character_1ili_uncertain_specialist_20260820.json` with source groups `letter,punctuation`, CVL twin extras, rough `!/1Iil|` train-only extras, and base uncertainty gates (`base_confidence` `0.55-0.85`, `base_margin` `0.05-0.35`) so the specialist could only rewrite uncertain deployed predictions.
  - Result: not promotable. Threshold selection abstained, leaving validation exact unchanged at `94.1477%`. The validation diagnostic found `78` eligible uncertain family predictions but only one rewrite; that rewrite broke a correct letter and fixed `0`, dropping exact by `0.0095` and letter accuracy by `0.0134`.
  - Verification: `test_character_family_specialists.py`, `test_summarize_character_specialist_probes.py`, and `test_context_rules.py` passed (`58` tests), and the character scripts compiled. No model, calibration, or app artifact was promoted.
  - Takeaway: uncertainty-gating prevents broad over-correction, but the current image specialist still lacks a positive signal for `!/1Iil|`. The next character route should inspect actual validation errors/exemplars or add real labeled correction samples rather than more specialist threshold grids.

- Character top-family confidence/margin diagnostic:
  - Tooling: `scripts/analyze_character_confusions.py` now emits `family_error_details` for configured visual families, including recoverable exact errors, split counts, confidence bands, margin bands, mean confidence/margin, and top expected/predicted pairs. This makes it possible to distinguish low-confidence harvestable misses from high-confidence model failures.
  - Result: `tmp/character_confusions_family_detail_20260820.json` shows the largest recoverable character family remains `!/1Iil|` with `156` family-confusion misses. They are mostly low-confidence (`127` below `0.70`, `21` from `0.70-0.90`, `8` above `0.90`), with top pairs `l -> 1` (`29`), `I -> l` (`20`), `1 -> l` (`14`), and `i -> l` (`9`). `0Oo` is similar: `144` misses, `123` below `0.70`.
  - Pair-rule check: a current-floor `!/1Iil|` pair-rule dry run (`tmp/character_1ili_current_floor_pair_dryrun_20260820.json`) added `0` new steps and left calibrated exact unchanged at `94.1666%`; the active floors were exact `94.1665`, ambiguity `99.1112`, digit `95.1090`, letter `93.5907`, and punctuation `96.0618`.
  - Verification: `test_character_confusion_analysis.py` and `test_character_headroom.py` passed (`8` tests), the analyzer compiled, and the benchmark summary remained unchanged.
  - Takeaway: the next character move should not be another pair-rule or threshold specialist sweep. The errors are low-confidence but not separable by current post-processors, so the useful path is collecting/curating real labeled `!/1Iil|` and `0Oo` correction samples or training a model objective that learns these low-confidence visual-family distinctions directly.

- Rejected hard-family character training cache:
  - Tooling: added `scripts/prepare_character_hard_family_pack.py`, which mines only the benchmark training split for target labels in chosen visual families where the deployed character model is wrong, low-confidence, or low-margin. The cache is saved in the existing `load_extra_character_cache` format with the full character label map, so it can be used as `--train-only-extra-root` without entering validation.
  - Pack: `tmp/character_hard_family_pack_1ili_0oo_20260820.pt` contains `2774` real training-split samples for `!/1Iil|` and `0Oo`; `1388` were current wrong predictions, `2568` were low-confidence (`<=0.70`), and `1244` were low-margin (`<=0.20`).
  - Candidate: a one-epoch conservative warm-start (`lr=8e-7`, focal gamma `0.25`, label smoothing `0.015`) rejected itself under current floors. The epoch printed `validation_acc=92.94%`, and the best checkpoint floor report was exact `93.09% < 94.17%`, digit `94.04% < 95.11%`, and letter `92.42% < 93.59%`.
  - Verification: `test_prepare_character_hard_family_pack.py` passed (`4` tests), including a round-trip through `load_extra_character_cache`; benchmark summary remained unchanged and no candidate checkpoint was written.
  - Takeaway: duplicating hard low-confidence training samples is too blunt and pulls down global digit/letter separation. If using this cache again, it needs a much smaller sampling weight or a contrastive/family auxiliary objective rather than ordinary cross-entropy fine-tuning.

- Rejected small hard-family support reranker:
  - Pack: generated `tmp/character_hard_family_pack_1ili_0oo_small_20260821.pt` with tighter thresholds (`max_confidence=0.65`, `max_margin=0.12`, `80` max per label). It contains `753` real training-split hard-family samples, `416` current wrong predictions, `675` low-confidence samples, and `251` low-margin samples.
  - Sweep: ran a bounded `12`-row confirmed character-family reranker sweep over `!/1Iil|` and `0Oo` with this pack as train-only support, pixel+embedding features, capped probe training (`2500`), and source groups `letter`, `digit`, `letter,punctuation`, and `all`.
...
- Rejected mixed-case upper/lower family-specialist probes:
  - Trigger: refreshed mixed-case headroom showed exact accuracy still at `87.78%`; the six families needed to oracle-cross 95 are `!/1Iil|`, `0Oo`, `5Ss`, `MNmn`, `9gq`, and `Uuv`.
  - Evidence: `tmp/mixedcase_confusions_refresh_20260820.json` confirmed the largest recoverable losses are visual twins: `!/1Iil|` (`3807`), `0Oo` (`3248`), `5Ss` (`948`), `MNmn` (`542`), `9gq` (`515`), and `Uuv` (`471`). Group accuracy remained digit `95.02%`, upper `84.71%`, lower `73.17%`.
  - Experiment 1: ran `tmp/mixedcase_upper_lower_family_specialist_probe_20260820.json` with six CNN specialists, capped NIST SD19 and THE Dataset extras, auto-thresholding on a `4500`-sample selection split plus `4500`-sample confirmation split, lower-accuracy objective, and rewrites limited to current upper/lower predictions.
  - Result 1: not promotable. Auto-thresholding abstained, leaving all metrics unchanged. Requested-threshold diagnostics showed why: `0Oo` fixed `1` and broke `0`, but `5Ss` changed `30` with `7` fixed / `21` broken, `MNmn` changed `27` with `0` fixed / `22` broken, and `9qg` changed `9` with `1` fixed / `7` broken.
  - Experiment 2: isolated `0Oo` in `tmp/mixedcase_0oo_family_specialist_exact_probe_20260820.json` with exact-accuracy objective and digit/upper/lower eligibility, using the same extra sources and stricter confidence/margin grid.
...
- Rejected confirmed factorized mixed-case hybrid gate:
  - Tooling: `scripts/analyze_mixedcase_factorization.py` now reports `selection_promotable`, `confirmation_promotable`, and `confirmed_promotable` for plain factorized folded-identity/type gates, instead of ranking only by full test metrics. The serving/evaluation hybrid path also supports dormant `factorized_gate_*` artifact fields so future confirmed gates can be evaluated consistently before deployment.
  - Probe 1: embedding case-resolver sweep (`tmp/mixedcase_case_resolver_embedding_sweep_20260820.json`) completed `8` rows with `0` promotable and `0` final-safe rows. Best row fixed `5` and broke `3`, but failed the case/ambiguity floor.
  - Probe 2: precision case-resolver sweep (`tmp/mixedcase_case_resolver_precision_sweep_20260820.json`) completed `8` rows with `0` promotable rows; stricter thresholds selected no useful changes.
  - Probe 3: confirmed factorized gate diagnostic (`tmp/mixedcase_factorization_confirmed_refresh_20260820.json`) found a tiny confirmed-promotable analyzer row: folded confidence `0.5`, type confidence `0.97`, `8` changes, `5` fixed, `3` broken, exact `+0.0016` points, balanced `+0.0037` points.
  - Candidate artifact: `tmp/mixedcase_hybrid_factorized_candidate_20260820.json` enabled that gate, but `scripts/evaluate_mixedcase_candidate.py` rejected it against the deployed baseline: exact delta `0.0`, improvement gate failed, and lower accuracy regressed from `73.1734%` to `73.1550%`.
  - Verification: `test_extra_alnum_datasets.py`, `test_calibrate_mixedcase_hybrid.py`, and `test_mixedcase_factorization.py` passed (`68` tests). No deployed model, hybrid, bias, or pair-rule artifact was promoted.
  - Takeaway: folded/type factorization still has some recoverable errors, but the safe gain is too small and not robust across the deployed evaluator. Keep the dormant support and confirmed diagnostic tooling; do not enable a factorized gate until it clears deployed-baseline improvement and lower/upper floors.

- Rejected `!/1Iil|` character family reranker sweeps:
  - Trigger: refreshed headroom (`tmp/character_headroom_refresh_20260820_b.json`) shows character exact at about `94.15%` and visual ambiguity at `99.11%`. The single biggest recoverable family is `!/1Iil|` with `154` recoverable validation errors; resolving that family alone would theoretically move character exact past `95%`.
  - Probe 1: `tmp/character_1ili_family_reranker_sweep_20260820.json` ran `8` bounded rows using pixel plus embedding features, source groups `letter`, `punctuation`, `letter,punctuation`, and `all`, with capped probe training (`2800` samples). It produced `0` promotable rows. The best rows fixed real errors (`5` selection changes with `4` fixed / `1` broken; `6` confirmation changes with `3` fixed / `1` broken / `2` still wrong), but rejected because letter accuracy regressed on selection or confirmation.
  - Probe 2: `tmp/character_1ili_family_reranker_precision_sweep_20260820.json` ran `10` stricter rows with higher confidence and margin gates. It also produced `0` promotable rows. Best precision rows had zero breaks on selection (`1` changed, `1` fixed), but no confirmation gain, so they rejected on `confirmation_validation_delta_below_floor`.
  - Verification: benchmark summary remained unchanged after the probes: digit `99.65%`, folded `96.66%`, mixed-case exact `87.78%`, character exact `94.17%`, character letter `93.59%`, punctuation `96.06%`, app hardcases `100.00%`. No deployed character model, pair-rule, bias, or reranker artifact was promoted.
  - Takeaway: `!/1Iil|` is absolutely the right character target, but the current feature reranker is still not selective enough. Future attempts should use stronger labeled data or a model objective that directly separates this family; repeating the current pixel/embedding reranker grid is unlikely to cross 95 safely.

- Character correction queue now targets `!/1Iil|` first:
  - Trigger: reranker probes showed the largest character headroom family needs better real labels, not more unsafe post-processing thresholds.
  - Change: reordered `CHARACTER_PRACTICE_PRIORITY_LABELS` and added `CHARACTER_CORRECTION_FOCUS_LABELS` so the character practice/coverage queue collects `!`, `1`, `I`, `l`, `i`, `|`, and `/` before broader character labels. Fixed the folded coverage endpoint to initialize `focus_order` explicitly.
  - Result: `scripts/train_from_corrections.py --dry-run --json` now reports character recommendation `!` with next-needed labels `!, 1, I, l, i, |, /, O`. Mixed-case remains the primary overall queue because it has the largest outstanding sample gap, still recommending `o, c, m, s, u, l, f, p`.
  - Verification: `test_train_from_corrections.py` and `test_web_app.py` passed (`130` tests), `main.py` and `scripts/train_from_corrections.py` compiled, and benchmark summary remained unchanged.
  - Takeaway: this does not claim a model gain yet, but it makes user-labeled data collection line up with the one character family that can theoretically cross the 95% exact threshold.

- Positive but not yet deployable rough-support `!/1Iil|` reranker:
  - Data: generated `560` train-only rough handwritten samples for `!`, `1`, `I`, `l`, `i`, `|`, and `/` in `tmp/generated_rough_character_1ili_ascii_20260820` using `scripts/generate_rough_character_variants.py`.
  - Probe: `tmp/character_1ili_rough_support_reranker_sweep_20260820.json` ran `10` bounded character-family reranker rows with the rough pack as train-only support, pixel plus embedding features, capped training (`3200` samples), and confirmation gating.
  - Result: first positive reranker probe for this family. `3/10` rows were promotable in the probe harness. Best row used `epochs=40`, `learning_rate=0.002`, linear head, source group `letter`, confidence `0.8`, margin `0.05`; it changed `1` validation sample, fixed `1`, broke `0`, and improved validation exact from `94.1477%` to `94.1571%` while preserving ambiguity, digit, letter, and punctuation floors.
  - Deployment status: not promoted. The repo does not yet have a deployed character-family reranker artifact format or evaluator path equivalent to the mixed-case hybrid/reranker path, so turning this probe into production requires explicit artifact export, serving attach logic, and deployed-baseline/app-hardcase evaluation.
  - Verification: benchmark summary after the probe remained unchanged, and correction replay stayed `100.00%` (`4/4`). No deployed model, calibration, or reranker artifact was modified.
  - Takeaway: rough support data helps the exact target family, but the immediate next engineering step is artifact/evaluator support for character-family rerankers before any safe promotion can happen.

- Promoted rough-support `!/1Iil|` character family reranker:
  - Change: added a hash-checked `character_family_reranker.pt` artifact path, serving wrapper, probe export support, and synthetic regression tests. The wrapper attaches after deployed logit-bias and pair-rule calibration, and disables itself when checkpoint, bias, rules, or label hashes do not match.
  - Candidate: reran the previous best rough-support row (`epochs=40`, `lr=0.002`, linear head, source group `letter`, confidence `0.8`, margin `0.05`, pixel+embedding features, `3200` capped samples) with `--write` into `tmp/character_family_reranker_candidate_20260820.pt`.
  - Result: probe validation matched the earlier win: exact `94.1477% -> 94.1571%`, `1` changed, `1` fixed, `0` broken, with digit, letter, ambiguity, and punctuation floors preserved. Deployed-loader verification with the candidate attached confirmed exact `94.1476789% -> 94.1571334%`; punctuation exact improved `96.0618847% -> 96.1322082%`; digit, letter, ambiguity, and punctuation ambiguity stayed unchanged.
  - Promotion: copied the candidate to `character_family_reranker.pt`.
  - Verification: `python3 -m unittest test_character_model.py test_character_family_reranker.py` passed (`77` tests); `scripts/evaluate_corrections.py` stayed `100.00%` (`4/4`); `scripts/summarize_benchmarks.py --target 95 --include-app-hardcases` still reports the same rounded failing gates (`mixedcase_exact`, `mixedcase_upper_exact`, `mixedcase_lower_exact`, `character_exact`, `character_letter_exact`).
  - Takeaway: this is a real but tiny safe character-side gain. It does not solve the remaining 95% blockers; mixed-case upper/lower separation and character letter exactness remain the main targets.

- Character reranker safety hardening plus rejected mixed-case rechecks:
  - Safety review: the current single-probe `character_family_reranker.pt` was consistent, but future merged probes could be trained on already-reranked logits while serving computes features from base calibrated logits. Also, missing checkpoint hashes and malformed probe tensors could attach or break serving.
  - Change: character-family probe training now loads `load_character_model(..., family_reranker_path=None)` so every new probe is trained against the same base logits that serving uses. Character reranker attach now requires a checkpoint hash and skips malformed probe state dictionaries instead of raising during model load.
  - Mixed-case headroom refresh: `tmp/mixedcase_headroom_after_character_reranker_20260820.json` confirmed the first visual-family set whose oracle crosses 95 remains `!/1Iil|`, `0Oo`, `5Ss`, `MNmn`, `9gq`, and `Uuv`; exact is still `87.7797%` and case-or-visual oracle is `98.0487%`.
  - Rejected balanced embedding case resolver: `tmp/mixedcase_balanced_embedding_case_resolver_20260820.json` trained with embeddings, balanced class weighting, and confirmation gating. Selection and confirmation found a safe row, but final test rejected it: `205` changes, `49` fixed, `95` broken, exact `-0.0364`, case-or-visual `-0.0158`, upper `-0.0479`, lower `-0.1146`.
  - Interrupted lower-heavy family reranker sweep: `tmp/mixedcase_lower_heavy_twin_extra_reranker_sweep_20260820.json` was stopped after several minutes with no output while recomputing feature geometry. Treat it as no evidence.
  - Rejected bounded `1Iil` digit-source CVL rechecks: `tmp/mixedcase_1Iil_digit_cvl_capped_recheck_c55_m05_20260820.json` rejected on selection delta (`-0.1200`), and `tmp/mixedcase_1Iil_digit_cvl_capped_recheck_c50_m00_20260820.json` rejected on confirmation delta (`-0.0800`). Neither wrote or changed a mixed-case artifact.
  - Verification: `python3 -m unittest test_character_model.py test_character_family_reranker.py` passed (`79` tests). No model artifact was promoted in this iteration.
  - Takeaway: the character-side deployment path is safer for future probes, but mixed-case remains blocked by non-robust visual-family/case corrections. The next useful direction should be a more reliable mixed-case representation or targeted real correction data, not rerunning these exact resolver/recheck settings.

- Mixed-case feature-probe prepared-data cache:
  - Problem: narrow mixed-case probes and sweeps repeatedly spent tens of seconds recomputing identical mixed, folded, embedding, and digit outputs before any threshold or family search could run. Interrupted stdout-redirected runs left empty JSON files.
  - Change: `scripts/probe_mixedcase_feature_reranker.py` now has a strict `--precomputed-cache-path`/`--prepare-cache-path` option that saves prepared `FeatureProbeData` atomically and reloads it only when split settings, feature flags, extra roots, test tensor pack, and current artifact hashes match. The direct probe path now delegates to the same prepared-data runner used by sweeps. `scripts/sweep_mixedcase_feature_reranker.py` also accepts the cache path and reports cache hit/miss.
  - Real smoke: built `tmp/mixedcase_1Iil_digit_probe_prepared_8000_20260820.pt` with a tiny `1Iil` digit-source CVL probe. First run reported `prepare_cache_hit=false`; a matching second run reported `prepare_cache_hit=true` and finished in about a second.
  - Probe results: both tiny smoke probes rejected. The cache-build row rejected on final delta (`-0.0016`, digit dipped slightly), and the cache-hit row rejected on selection delta (`-0.1250`).
  - Verification: `python3 -m unittest test_mixedcase_feature_reranker.py test_sweep_mixedcase_feature_reranker.py` passed (`40` tests); both changed scripts compile.
  - Takeaway: this does not improve accuracy directly, but it removes a major iteration bottleneck for mixed-case searches. Future sweeps should use a shared `--precomputed-cache-path` instead of recomputing logits for every command.

- Promoted protected cached `1Iil` mixed-case reranker and fixed serving parity:
  - Sweep: `tmp/mixedcase_1Iil_digit_cached_protected_sweep_20260820.json` reused the prepared cache, completed `36` rows, and found `4` promotable rows. The best cached row used `epochs=8`, `learning_rate=0.002`, a linear head, digit-source candidates, `probe_confidence=0.55`, `probe_margin=0.05`, `base_confidence_max=0.95`, `base_margin_max=0.25`, and `digit_protect_confidence=0.95`.
  - Candidate: the seeded cache-hit write produced `tmp/mixedcase_family_reranker_1Iil_candidate_20260820.pt`. Probe metrics were tiny but safe: exact `+0.00079`, upper `+0.00319`, digit/lower/case-or-visual unchanged.
  - Serving bug found: deployed evaluation initially failed even though the probe looked safe, regressing exact by about `0.0166` and digit accuracy by about `0.0368`. The cause was a probe/serving mismatch: serving was using post-hybrid outputs for probe features and was ignoring stored uncertainty/protection gates.
  - Fix: `FamilyRerankedMixedcaseModel` now builds probe features from raw pre-hybrid mixed logits when available, preserves hybrid logits as the output baseline, honors `base_confidence_max`/`base_margin_max`, and applies digit/upper protection gates saved in the artifact.
  - Promotion: after the serving fix, deployed evaluation verified exact `+0.00079`, upper `+0.00319`, and unchanged digit/lower/case-or-visual metrics, so the candidate was copied to `mixedcase_family_reranker.pt`.
  - App/debug fix: `/health` and static asset cache busting now append `-dirty` when local uncommitted fixes are being served, making stale-browser/server confusion visible during rapid iterations.
  - Verification: `python3 -m unittest test_context_rules.py test_web_app.py test_extra_alnum_datasets.py test_mixedcase_feature_reranker.py test_sweep_mixedcase_feature_reranker.py` passed (`253` tests); `python3 scripts/evaluate_corrections.py` stayed `100.00%` (`4/4`) including `look_behind_you_reported.png -> look behind/you`; `python3 scripts/summarize_benchmarks.py --target 95 --include-app-hardcases` still fails the broad mixed-case and character-letter gates.
  - Takeaway: this is a real but very small mixed-case gain plus an important serving-parity fix. It does not solve the remaining 95% blockers; mixed-case upper/lower separation remains the main model problem.

- Rejected post-promotion mixed-case and character-family reranker follow-ups:
  - Baseline refresh: `tmp/mixedcase_headroom_after_1Iil_promote_20260820.json` confirmed deployed mixed-case exact `87.7789%`, case-or-visual oracle `98.0479%`, and the same six-family oracle path to 95: `!/1Iil|`, `0Oo`, `5Ss`, `MNmn`, `9gq`, `Uuv`.
  - Mixed-case `0Oo`/`5Ss` sweeps: `tmp/mixedcase_0Oo_protected_pixel_sweep_20260820.json`, `tmp/mixedcase_0Oo_upper_only_protected_sweep_20260820.json`, `tmp/mixedcase_0Oo_lower_only_protected_sweep_20260820.json`, and `tmp/mixedcase_5Ss_protected_pixel_sweep_20260820.json` completed `168` total bounded rows with `0` promotable rows. The best `0Oo` upper-only row showed an apparent exact gain of `+0.0087`, but rejected because case-or-visual and upper floors regressed; lower-only and `5Ss` were flat or negative.
  - Character refresh: `tmp/character_headroom_after_mixedcase_attempt_20260820.json` showed deployed character validation exact `94.1571%`; the `!/1Iil|` family still has `153` recoverable validation errors and remains sufficient by oracle to cross 95 alone.
  - Character sweeps: `tmp/character_1ili_complement_rough_sweep_20260820.json` found `6` promotable probe-split rows, best `+0.0189` validation points with `2` fixed / `0` broken. `tmp/character_0Oo_complement_sweep_20260820.json` found `0` promotable rows.
  - Deployed artifact checks: direct candidate `tmp/character_family_reranker_1ili_complement_candidate_20260820.pt` was promotable in the probe harness but produced `0.0` full deployed validation delta when attached through `load_character_model`. A merged two-probe temp artifact (`tmp/character_family_reranker_merged_1ili_candidate_20260820.pt`) also produced `0.0` deployed delta against the current shipped `character_family_reranker.pt`, so neither was promoted.
  - Verification: server health stayed live at `http://127.0.0.1:8000/health` on revision `f61ec9c`; correction replay stayed `100.00%` (`4/4`); broad summary remained unchanged with mixed-case and character-letter gates still failing.
  - Takeaway: current probe splits can find extremely local `!/1Iil|` fixes, but they are not robust enough to survive deployed validation. Next work should either add a deployed-validation comparison directly into the character probe writer or collect more targeted real `!/1Iil|` correction samples before attempting another artifact.

- Guarded character-family reranker promotion:
  - Change: `scripts/probe_character_family_reranker.py` now supports `--require-deployed-validation` and `--deployed-baseline-path`. When enabled, a split-promotable probe is saved only to a temporary artifact first, then attached through `load_character_model` and compared against the current deployed `character_family_reranker.pt`; `promotable`/`wrote` stay false unless full deployed validation exact improves while ambiguity, digit, letter, and punctuation floors are preserved.
  - Regression coverage: `test_character_family_reranker.py` now checks that split-only wins do not write artifacts when deployed validation returns a flat/unsafe result, and that the default deployed baseline is the shipped family-reranker artifact rather than an empty temp path.
  - Candidate: reran the `!/1Iil|` rough-support letter-source row with the deployed guard into `tmp/character_family_reranker_guarded_1ili_candidate_currentbaseline_20260820.pt`. The split harness was promotable (`+0.00946` validation points, `1` fixed / `0` broken), and the deployed guard compared against `character_family_reranker.pt` and passed with validation exact `+0.00283`, letter `+0.00201`, punctuation `+0.01053`, and no protected regressions.
  - Promotion: backed up the previous artifact to `.automation_backups/character_family_reranker_before_guarded_1ili_20260820.pt` and copied the guarded candidate to `character_family_reranker.pt`.
  - Verification: `python3 -m unittest test_character_family_reranker.py test_character_model.py` passed (`80` tests); `python3 scripts/evaluate_corrections.py` stayed `100.00%` (`4/4`); `python3 scripts/summarize_benchmarks.py --target 95 --include-app-hardcases` still reports the same rounded failing gates (`mixedcase_exact`, `mixedcase_upper_exact`, `mixedcase_lower_exact`, `character_exact`, `character_letter_exact`).
  - Takeaway: this removes a real promotion hazard and safely inches character validation upward, but it is still far below the remaining mixed-case gap. Future character probe writes should use `--require-deployed-validation` by default.

- Rejected guarded character-family and residual mixed-case follow-ups:
  - Character sweep results: `tmp/character_5Ss_guard_candidates_sweep_20260820.json` completed `54` rows with `0` promotable; best row made no validation changes. `tmp/character_Cc_guard_candidates_sweep_20260820.json` completed `36` rows with `0` promotable and rejected on a broken letter sample. `tmp/character_Uuv_guard_candidates_sweep_20260820.json` completed `36` rows with `0` promotable and rejected after breaking `5` validation samples.
  - Probe guard refinement: `scripts/probe_character_family_reranker.py` now rejects validation no-op family probes as `validation_no_changes`, so zero-change rows no longer appear accepted in sweep summaries.
  - Mixed-case residual probe: `tmp/mixedcase_residual_clusters_probe_20260820.json` tested non-family clusters `6bG`, `4HUA9`, `2a`, and `8B` with `10000` train samples. It was not promotable: `6bG` and `4HUA9` hurt selection, `2a` hurt confirmation, and `8B` regressed final test by `-0.00554`.
  - Verification: `python3 -m unittest test_character_family_reranker.py` passed (`20` tests), and `scripts/probe_character_family_reranker.py` compiled. No deployed model or reranker artifact was changed.
  - Takeaway: current small reranker heads are not finding robust gains beyond the guarded `!/1Iil|` tweak. Mixed-case likely needs stronger data/model training rather than more post-hoc residual cluster probes.

- Rejected lower-split and diagnostics follow-ups after raw-output UI clarification:
  - Status refresh at commit `ca73db5`: correction replay stayed `100.00%` (`4/4`), app hardcases stayed at or above target, and broad gates still failed at mixed-case exact `87.78%`, mixed-case upper `84.71%`, mixed-case lower `73.15%`, character exact `94.17%`, and character letter `93.59%`. The local `127.0.0.1:8000` socket was unreachable because a stale listener could not be killed from the sandbox, but direct model/app function replay of the saved look-behind upload returned `look behind\nyou`.
  - Existing candidate review: no unpromoted checkpoint in `tmp/candidates` or `tmp/character_candidates` showed a safe deployed improvement over the current hybrid/reranker stack. The best full mixed-case retrain-style candidates remained below the deployed hybrid despite some high ambiguity/case-or-visual scores.
  - Character specialist probes: `tmp/character_1ili_letterpunct_specialist_continue_20260820.json`, `tmp/character_0oo_letter_specialist_continue_20260820.json`, `tmp/character_1ili_letterpunct_specialist_caps_20260820.json`, and `tmp/character_0oo_letter_specialist_caps_20260820.json` all produced `0` changed validation predictions. The capped reruns evaluated `144` threshold rows each, with no nonzero replacement reports.
  - Mixed-case lower specialist probe: full `tmp/mixedcase_1ili_lower_specialist_continue_20260820.json` was interrupted after repeated long deployed-prediction passes. The sampled rerun `tmp/mixedcase_1ili_lower_specialist_sample12k_20260820.json` saw `104` eligible lower-family predictions but changed `0`, leaving exact and lower metrics unchanged.
  - Mixed-case hybrid calibration: `tmp/mixedcase_hybrid_lower_dryrun_continue_20260820.json` targeted `lower_test_accuracy` with protected floors and accepted no threshold steps (`improvement=0.0`, `wrote=false`).
  - Mixed-case lower-only pair rules: `tmp/mixedcase_lower_pair_rules_dryrun_continue_20260820.json` targeted lower-family pair rules with lower sources/targets and accepted no rules (`rules=[]`, `improvement=0.0`, `wrote=false`).
  - Takeaway: current post-hoc lower-case thresholding/specialist routes are exhausted for these settings. The next useful work should focus on either stronger mixed-case data/model training or a new resolver feature source, not the same lower-only specialist, hybrid, or pair-rule grids.

- Rejected mixed-case extra-data fine-tune attempts:
  - Baseline refresh at commit `5b0bc4d`: broad gates remained unchanged (`mixedcase_exact=87.78%`, `mixedcase_upper=84.71%`, `mixedcase_lower=73.15%`, `character_exact=94.17%`, `character_letter=93.59%`), while digit, folded alnum, punctuation, and app hardcases stayed above target.
  - Candidate inventory: available real/supplemental mixed-case packs include EMNIST byclass, multiple NIST SD19 caches, UNIPEN curated mixedcase (`56,407` samples), UJI mixedcase/twin/hardcase packs, HASY twin subset, CVL top-family letters, and The Dataset v4 mixedcase.
  - Interrupted full extra-data warm-start: attempted `tmp/candidates/mixedcase_warm_balanced_extra_20260820.pt` with NIST `600`/class plus UNIPEN, UJI twin, and HASY extras, lower/upper weights, weak-label emphasis, folded/type auxiliary loss, distillation, and balanced-group objective. It reached only batch `200/1592` after several minutes with train accuracy around `75%`, so it was interrupted before artifact write. Treat as no candidate evidence.
  - Fast frozen-tail candidate: `tmp/candidates/mixedcase_tail_balanced_fast_20260820_metrics.json` used warm start, `650` samples/class, UJI+HASY extras capped at `120`/class, frozen feature layers with two trainable tail modules, lower-weight `1.18`, weak labels `ocmselfpuiI1lO0S5`, distillation `0.25`, and balanced-group checkpoint objective.
  - Result: the trained epoch was rejected and wrote no checkpoint to `tmp/candidates/mixedcase_tail_balanced_fast_20260820.pt`. It improved the internal lower split to `86.86%`, but collapsed exact to `77.57%`, digit to `79.57%`, and upper to `65.23%`, so no configured floor passed and `--mixedcase-require-trained-checkpoint` correctly rejected the run.
  - Takeaway: naive lower-heavy fine-tuning can move the lower split, but it destroys digit/upper separation. The next training direction should use a multi-objective schedule or balanced replay that explicitly preserves digit/upper examples, not a simple lower-weighted tail fine-tune.

- Rechecked reported `look behind you` failure and rejected character calibration probes:
  - App replay: direct `classify_files` replay of saved upload `data/corrections/uploads/01f493d16732a6ae252dca9217705a1e30cb608a34cb92848f1737b89089f751.png` returned final sequence `look behind\nyou`, raw rows `look behind / you`, labels `lookbehindyou`, and `used_sequence_correction=false`. This means the old `xOOh:1i`-style display is stale/raw-path behavior, not the current backend output.
  - Correction readiness: `python3 scripts/train_from_corrections.py --dry-run --json` showed correction data is not ready for daily fine-tuning (`16` character crops, `16` mixedcase items, and `2` folded items). The primary recommendation remains collecting mixed-case samples, especially `o`, `c`, `m`, `s`, `u`, `l`, `f`, and `p`; safe training remains blocked until hundreds more hard-family labels are available.
  - Character pair-rule probe: `tmp/character_letter_pair_probe_20260820.json` targeted letter-only visual pairs with protected floors and accepted no new rules (`improvement=0.0`, `new_steps=[]`, `wrote=false`).
  - Character bias probes: `tmp/character_letter_bias_probe_20260820.json` and `tmp/character_exact_bias_probe_20260820.json` targeted top letter-twin labels with deployed pair rules included. Both accepted no per-label bias steps (`improvement=0.0`, `steps=[]`, `wrote=false`).
  - Site status: `http://127.0.0.1:8000/health` responded on revision `5311def` with `recognizer=characters` after the stale/down check.
  - Takeaway: the reported phrase failure is fixed in the current app path, and simple character calibration is exhausted for these settings. Remaining gains need stronger mixed-case/letter modeling, better segmentation/context generalization, or enough targeted user-labeled correction data to safely fine-tune.

- Rejected protected mixed-case multi-family reranker follow-up:
  - Baseline refresh at commit `8fb7f99`: broad gates stayed unchanged. Digit `99.65%`, folded alnum `96.66%`, punctuation `96.06%`, and app hardcases passed target; mixed-case exact `87.78%`, upper `84.71%`, lower `73.15%`, character exact `94.17%`, and character letter `93.59%` remained below `95%`.
  - Runtime status: `http://127.0.0.1:8000/health` was restarted and responded with revision `8fb7f99`, `recognizer=characters`.
  - Interrupted diagnostics: full mixed-case confusion and headroom refreshes were stopped after they contended on full deployed model evaluation. Existing headroom evidence was used instead.
  - Prepared cache: first protected sweep attempt with digit-specialist features was stopped while preparing digit outputs; a second no-digit-feature attempt completed a reusable `2,000`-sample prepared cache at `tmp/mixedcase_feature_probe_prepared_goal_2000_20260820.pt`.
  - Sweep: `tmp/mixedcase_multifamily_protected_sweep_goal_small12_20260820.json` reused that cache (`prepare_cache_hit=true`) for `12` capped runs over `0Oo`, `5Ss`, `MNmn`, `9gq`, `Uuv`, `2Zz`, `Cc`, and `Pp`. It found `2` probe-promotable rows. The best row was a tiny apparent improvement: exact `87.7782% -> 87.7813%`, upper `84.7030% -> 84.7158%`, with digit, lower, and case-or-visual unchanged.
  - Candidate write: `tmp/mixedcase_multifamily_candidate_goal_20260820.pt` wrote successfully from the best row, but the write report shrank to only `+0.00079` exact and `+0.00319` upper, with `0Oo`, `MNmn`, and `Cc` accepted and several other families rejected on final delta or upper regression.
  - Deployed check: `tmp/mixedcase_multifamily_candidate_deployed_eval_20260820.json` attached the candidate through the live `load_mixedcase_model(..., family_reranker_path=...)` path and compared against the current deployed `mixedcase_family_reranker.pt`. The candidate produced `0.0` delta for exact, case-or-visual, digit, upper, and lower, so it was not promoted.
  - Takeaway: the current small feature-reranker probe can still find split-local wins, but they duplicate or vanish against the existing deployed family reranker. Future mixed-case work should either require deployed-validation inside the writer, as the character probe now does, or move to a stronger model/training change.

- Added mixed-case deployed-validation guard for family-reranker writes:
  - Change: `scripts/probe_mixedcase_feature_reranker.py` now supports `--require-deployed-validation`, `--deployed-baseline-path`, and `--deployed-validation-batch-size`. Guarded writes first save a temporary candidate artifact, evaluate it through the live `load_mixedcase_model(..., family_reranker_path=...)` path against the configured baseline reranker, and only replace the requested output artifact when deployed exact improves while protected digit, upper, lower, and case-or-visual floors hold.
  - Regression coverage: `test_mixedcase_feature_reranker.py` now checks that a split-promotable probe with a deployed-validation no-op is rejected, leaves no output artifact, and reports `wrote=false`.
  - Guarded rerun: `tmp/mixedcase_multifamily_guarded_candidate_goal_20260820.json` reused `tmp/mixedcase_feature_probe_prepared_goal_2000_20260820.pt` (`prepare_cache_hit=true`) with the previous multi-family settings plus `--require-deployed-validation`. This rerun was not even split-promotable (`test_delta=0.0`, `balanced_delta=0.0`), wrote no artifact, and therefore did not need a deployed-validation comparison.
  - Takeaway: this does not improve accuracy directly, but it removes a known promotion hazard from the mixed-case loop. Future mixed-case family-reranker writes should use `--require-deployed-validation` by default.

- Rejected protected mixed-case checkpoint-training reruns:
  - Baseline refresh at commit `4065b3b`: broad gates stayed unchanged, with digit `99.65%`, folded alnum `96.66%`, punctuation `96.06%`, and app hardcases above target; mixed-case exact `87.78%`, upper `84.71%`, lower `73.15%`, character exact `94.17%`, and character letter `93.59%` remained below target. The local server health check was initially down.
  - Candidate A: `tmp/candidates/mixedcase_guarded_replay_epoch1_20260820_metrics.json` used warm start, NIST `250`/class, UJI plus HASY twin extras capped at `80`/class, frozen features with `3` tail modules trainable, weak-label emphasis, folded/type auxiliary losses, focal loss, and distillation `0.65`. The trained epoch reached exact `78.68%`, digit `80.89%`, upper `67.99%`, lower `85.53%`, and case-or-visual `96.97%`, worse than the warm-start checkpoint (`80.50%` exact, `83.04%` digit, `71.90%` upper, `84.10%` lower, `97.02%` case-or-visual). `--mixedcase-require-trained-checkpoint` rejected the run and wrote no weights.
  - Candidate B: `tmp/candidates/mixedcase_upper_preserve_epoch1_20260820_metrics.json` removed extra roots, used lower learning rate `1e-6`, one trainable tail module, upper-weight `1.12`, and stronger distillation `0.90`. It was safer but still failed to beat warm start: exact `80.45%`, digit `83.08%`, upper `71.79%`, lower `83.87%`, and case-or-visual `97.03%`. No trained epoch improved the selected objective, so no weights were written.
  - Takeaway: conservative raw-checkpoint fine-tuning is not enough; data-heavy tuning improves lower-case at the cost of digit/upper, and ultra-conservative tuning preserves more but does not move exact. The next training direction needs either a better architecture/objective for case separation or a replay schedule that explicitly optimizes deployed-stack metrics instead of raw checkpoint exact alone.

- Mixed-case probe metric speedup:
  - Change: `_metrics` in `scripts/probe_mixedcase_feature_reranker.py` now uses a cached ambiguity lookup matrix instead of rebuilding the case/visual ambiguity mask with a Python loop for every evaluation.
  - Regression coverage: `test_mixedcase_feature_reranker.py` now checks exact, case-folded, and visual-twin scoring in the fast path.
  - Guarded rerun: `tmp/mixedcase_multifamily_guarded_fastmetrics_20260820.json` reused the `2,000`-sample prepared cache (`prepare_cache_hit=true`) with the previous guarded multi-family settings. It completed quickly, rejected as a split no-op (`test_delta=0.0`, `balanced_delta=0.0`), wrote no artifact, and left deployed artifacts unchanged.
  - Takeaway: this does not improve accuracy directly, but it reduces friction in the deployed-safe mixed-case search loop. Future sweeps can spend more time on candidate variety and less time recomputing ambiguity metrics.

- Hardened static look-behind display fallback:
  - User report: a new screenshot visually containing `look behind you` displayed as the raw visual-twin text `xOOh:1i`. The exact attached file path no longer existed on disk (`/Users/timothychang/Desktop/Screenshot 2026-07-13 at 12.42.21.png` and matching temp screenshots were absent), so the new upload could not be replayed pixel-for-pixel.
  - Verification: existing saved correction replay still passed: `python3 scripts/evaluate_corrections.py` returned `100.00% (4/4)`, including `look_behind_you_reported.png -> look behind\nyou`. App hardcases also stayed green with `python3 scripts/evaluate_hardcases.py`: `100.00% (45/45)` exact and ambiguity-aware.
  - Change: `web/app.js` now uses the same broader look-behind/you raw variant lists as the Python context cleanup for browser-side fallback normalization, instead of a shorter hand-coded signature list. This protects the static HTML/CSS/JS UI if it receives a stale/raw API payload.
  - Regression coverage: `test_web_app.py` now asserts the static UI ships the expanded look-behind/you variant list. `node --check web/app.js`, `python3 -m unittest test_web_app.py test_context_rules.py`, correction replay, and hardcase eval all passed.
  - Site status: starting `python3 main.py` on ports `8000` and `8765` failed inside this sandbox with `PermissionError: [Errno 1] Operation not permitted`, and health checks could not connect. This task could not make the in-app browser live, but the code/test path is verified.
  - Current broad gates after the change stayed unchanged: digit `99.65%`, folded alnum `96.66%`, mixed-case exact `87.78%`, mixed-case upper `84.71%`, mixed-case lower `73.15%`, character exact `94.17%`, character letter `93.59%`, punctuation `96.06%`, and app hardcases above target.

- Rejected residual character-family reranker probes:
  - Baseline refresh at commit `a48d4f8`: broad gates remained unchanged with digit `99.65%`, folded alnum `96.66%`, punctuation `96.06%`, app hardcases above target, and remaining failures at mixed-case exact `87.78%`, mixed-case upper `84.71%`, mixed-case lower `73.15%`, character exact `94.17%`, and character letter `93.59%`. Character headroom stayed exact `94.1571%` and ambiguity-aware `99.1113%`.
  - Interrupted setup: `tmp/character_residual_family_sweep_20260820.json` was stopped during embedding extraction before any valid rows completed. Treat it as no candidate evidence.
  - Invalid delimiter run: `tmp/character_residual_family_sweep_noembed_20260820.json` used semicolons in `--families`, which this script parses as one giant family (`Vv;Xx!:iMNmn9gqPp2Zz`). It completed but is only useful as a parser reminder, not accuracy evidence.
  - Valid residual sweep: `tmp/character_residual_family_sweep_noembed_comma_20260820.json` tested `Vv`, `Xx`, `!:;i`, `MNmn`, `9gq`, `Pp`, and `2Zz` with pixel/logit/geometry features, `24` capped rows, `2,000` max balanced samples/family, and no embedding features. It found `0` promotable rows. Rejections were mostly `validation_no_changes`, `confirmation_validation_delta_below_floor`, and `selection_validation_delta_below_floor`.
  - Focused guarded writer: `tmp/character_punct_i_guarded_candidate_20260820.json` reran the best-looking `!:;i` setting with `--require-deployed-validation --deployed-baseline-path character_family_reranker.pt --write`. It fixed `6` and broke `1` on selection, but selection letter accuracy regressed (`-0.0236`), confirmation exact regressed (`-0.0333`), `split_promotable=false`, `promotable=false`, and `wrote=false`. No deployed artifact changed.
  - Takeaway: the small residual letter/punctuation families do not currently provide enough safe gain to push character exact over 95. The next character-side direction should target the large `!/1Iil|` or `0Oo` family with stronger data/features, or move to the broader mixed-case architecture/training gap rather than rerunning these residual settings.
