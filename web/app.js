...
function renderPracticeCoverage(payload) {
  ...
  if (focusItems.length > 0) {
    ...
    const focusPercent = Number(payload.focus_coverage_percent ?? 0);
    focus.append(makeElement("span", "", `Focus (${focusNeeded} samples, ${focusPercent.toFixed(1)}%)`));
    const batchLabels = Array.isArray(payload.recommended_batch_labels)
      ? payload.recommended_batch_labels.map((label) => text(label)).filter((label) => label)
      : focusItems.map((item) => item.label);
    if (batchLabels.length > 0) {
      focus.append(makeElement("span", "", batchLabels.join(" ")));
    }
...
function renderRows(result, panel) {
  ...
  if (rawSequence && rawSequence !== displaySequence) {
    ...
    if (rawRows !== cleanedRows) {
      ...
      rawWrap.append(makeElement("summary", "", "Diagnostics only: model raw read"));
      const rawBody = makeElement("div", "context-output raw-output-body");
      rawBody.append(makeElement("code", "row-chip", `ignore as answer - raw model boxes: ${rawRows}`));
      rawBody.append(makeElement("code", "row-chip final-answer-chip", `final answer used by app: ${cleanedRows}`));
